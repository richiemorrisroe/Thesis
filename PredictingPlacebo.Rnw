...
randf.train <- train(PlacResp~., data=iatandexpfull.train, method="rf", trControl=mytrain, tuneGrid=data.frame(.mtry=1:10), preProcess=c("center", "scale", "PCA"))
randf.pred <- predict(randf.train, iatandexpfull.test)
@ 

<<nbtrain, echo=FALSE, results=hide>>=
nb.train <- train(PlacResp~., data=iatandexpfull.train, method="nb", trControl=mytrain, tuneGrid=data.frame(.usekernel=c(TRUE, FALSE), .fL=0:1), preProcess=c("center", "scale", "PCA"))
@
...
svm.train <- train(PlacResp~., data=iatandexpfull.train, method="svmLinear", trControl=mytrain, tuneGrid=data.frame(.C=1:10), preprocess=c("center", "scale", "pca"))
@ 

<<svmradialtrain, echo=FALSE, results=hide>>=
svmradial.train <- train(PlacResp~., data=iatandexpfull.train, method="lssvmRadial", trControl=mytrain, tuneGrid=data.frame(.sigma=0:10), preProcess=c("center", "scale", "pca"))
@ 
