...
One line of evidence which supports the idea that implicit measures may prove useful in the prediction of placebo  is the finding that the Implicit Association Test (IAT) is better at predicting spontaneous behaviour than explicit measures \cite{Conner2005,Hofmann2005}. The Conner \textit{et al} research was an experience sampling study which found that implicit association tests of heteronomous versus autonomous motivation were a better predictor of affect measured at random times throughout the study, while deliberative behaviours were better predicted by the explicit self report measures. The Hofmann study focused on shy behaviour, and determined that scores on a shyness IAT were better at predicting responses to the Trier Social Stress Task. Additionally, Richetin \textit{et al}  found that scores on a fruit-sweets IAT predicted the choice between these two items at the end of an unrelated study \cite{Richetin2007}. 

The IAT outperforms explicit measures in some domains \cite{Greenwald2009} and these domains tend to be where there is little conscious deliberation or reflection upon the matter concerned. The placebo effect is the example \textit{par excellance} of a undeliberated and spontaneous phenomenon, as no one chooses to have such a response, as far as is known. Thus, we can take this as supporting evidence that implicit measures should predict the placebo response more effectively. 

In the light of evidence showing that the placebo effect can be mediated by unconscious influences, and that the implicit association test can reliably measure these kinds of associations, it seems natural to propose that the IAT may be a useful predictor of the response to placebo in healthy volunteers. 
...
\item The Life Orientation Test, Revised \cite{Scheier1994}. Reliability in the sample population was assessed in three other studies, and averaged at  $\alpha = 0.84$

\item The Mindful Attention Absorption Scale \cite{brown2003benefits}. This fifteen item six point scale is commonly used to measure Mindfulness levels in non clinical samples. Reliability analyses carried out in the sample population showed that $\alpha = 0.9$. 
...
\item The majority ($n=79$) of participants completed a scrambled sentence taks, to prime them to cooperate with the experimental procedure. The rest of the sample acted as a control for this procedure. Participants were randomised to either a prime or no prime condition seperately from the randomisation into treatment group. 
\end{itemize}
...
<<gendcond, echo=FALSE, results=tex>>=
gend.cond <- print(xtable(with(Iatandexpmeasures, table(Gender, Condition)), caption="Gender Breakdown by Condition", label="tab:gendcond"))
@ 

Table \ref{tab:gendcond} shows the breakdown of participants' gender by their condition. 

<<gendprime, echo=FALSE, results=tex>>=
prime.gend <- print(xtable(with(Iatandexpmeasures, table(Gender, Prime)), caption="Gender Breakdown by Priming Condition", label="tab:primegend"))
@ 

%% \begin{figure}
%%   \includegraphics[totalheight=0.5\textheight]{flowchart}
%%   \caption{Flowchart of Gender and Assignment to Condition}
%%   \label{fig:flowchart}
%% \end{figure}
...
Firstly, the implicit measures were examined to ensure that they were not contaminated by general processing speed or by method variance. The correlations between in each block are never higher than 0.4, suggesting that the blocks are not contaminated by general processing speed. There was some autocorrelations between responses in adjacent blocks, but this was marginal. 


%% \begin{figure}
<<tcqmeanresp, echo=FALSE, results=hide, eps=TRUE, pdf=TRUE>>=
...
<<cormatrixtcqiat, echo=FALSE, results=hide>>=
...
%% \begin{figure}
<<tcqmeanresp, echo=FALSE, results=hide>>=
...
@ 
<<buildplot, echo=FALSE, results=hide>>=
pdf(file="corriattcqopt.pdf")
...
\begin{figure}
<<corriattcqopt, echo=FALSE, fig=TRUE, pdf=TRUE, eps=TRUE, png=TRUE, jpg=TRUE, eps=TRUE>>=
corr.iat.tcq.opt.pl <- plotmatrix(iat.block.merge2[,2:length(iat.block.merge2)])+geom_smooth(method="lm")
print(corr.iat.tcq.opt.pl)
@   
...
<<buildprimeplot, echo=FALSE, results=hide>>=
plac.prime.pl <- ggplot(Iatandexpmeasures, aes(x=PlacResp))+geom_histogram()+facet_grid(.~Prime)
@ 


\begin{figure}
<<placprimeplot, echo=FALSE, fig=TRUE, pdf=TRUE, png=TRUE, jpg=TRUE, eps=TRUE>>=
plac.prime.pl <- ggplot(Iatandexpmeasures, aes(x=PlacResp))+geom_histogram()+facet_grid(.~Prime)
print(plac.prime.pl)
@   
...
\begin{figure}
<<ggplothistcond, echo=FALSE, fig=TRUE, eps=TRUE, pdf=TRUE, png=TRUE, jpg=TRUE,eps=TRUE>>=
...
<<buildconddensity, echo=FALSE, results=hide>>=
pdf(file="opthistcond.pdf")
...
\begin{figure}
<<optiatcorrsurv, echo=FALSE, fig=TRUE, eps=TRUE, pdf=TRUE, png=TRUE, jpg=TRUE,eps=TRUE>>=
optiat.histcond <- ggplot(Iatandexpmeasures, aes(x=OptIAT.Mean))+geom_density()+facet_grid(.~PlacResp)
print(optiat.histcond)
@   
...
\begin{figure}
<<ggplotplacyesno, echo=FALSE, fig=TRUE, eps=TRUE, pdf=TRUE, png=TRUE, jpg=TRUE,eps=TRUE>>=
...
\begin{figure}
  \centering
<<ggplot3wayinter, echo=FALSE, fig=TRUE, eps=TRUE, pdf=TRUE, jpg=TRUE,eps=TRUE>>=
...
placmod.int <- glm(PlacResp~TCQIAT.Mean*OptIAT.Mean*LOTR, data=Iatandexpmeasures, family=binomial(link="logit"))
...
placmod.int2 <- glm(PlacResp~(TCQIAT.Mean*OptIAT.Mean*LOTR)^2, data=Iatandexpmeasures, family=binomial(link="logit"))
...
The major aim of this study was to assess if the placebo response could be  achieved in a placebo analgesia study without deception. 
This finding is consistent with the findings of Kapthcuk  \textit{et al} \cite{kaptchuk2010placebos} that people given an open placebo will experience a placebo response. Even when the administration of a deceptive placebo is a possibility, the open placebo still proves more effective. 

Secondly, there appears to be a relationship between implicit measures (assessed using the IAT) and the placebo response. However, this effect needs to be replicated due to issues with multicollinearity making it difficult to establish whether the effect is real or not. A study with greater power would be necessary to establish whether or not this effect is replicable. 

There are a number of potential explanations for this unusual finding. Firstly, the primary researcher's thesis was on placebo, and he possessed a strong belief that placebos actually work. This could easily have been communicated to the participants \cite{Blasi2001}. 

Secondly, the experimenter was not comfortable with the deception involved in informing participants that they were about to receive a real drug, and this may have been communicated to the participants also. Additionally, the lack of any side-effects or sensory stimulus from the inert cream used in this study may have cued participants to believe that they were not getting a particularly good medication. It may also have been the case that the use of the term ``recently approved'' for the deceptive placebo administration may have had an impact on the response to this placebo, though this phrase is common in placebo research \cite{Whalley2008}. 

Finally, it appears that priming manipulations are as effective in placebo analgesia as they have been in other areas of social cognition \cite{bargh2012automaticity}. This is important as priming is a less resource intensive way to increase the size of placebo effects, which is important if these effects are to be studied effectively. 

Some strengths of the study are as follows. Firstly, all measures had previously been validated on samples from the same population, suggesting that these measures were appropriate. Secondly, the one experimenter design ensured that effects relating to the provider of the treatment remained constant. 
...
Some implications of the results are as follows. Firstly, open placebo appears to be effective in some situations, and further research is required to delinate these. Secondly, priming appears to be a valid way to increase the size and frequency of placebo effects in experimental studies. Ideally, this hypothesis could be tested in a randomised controlled trial, as this hypothesis has not yet been tested using a double blind methodology. Finally, this research would seem to suggest that in some cases, placebo prescription may be ethical and effective. 
