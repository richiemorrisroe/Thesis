...
% So, having reviewed the placebo effect and the implicit association test above and examined the approaches taken to measurement within psychology, we are now in a position to examine the main experimental hypothesis of this thesis: that the placebo effect can be predicted by the use of reaction time based implicit measures. Below, the reasons for believing this shall be set out, and in the next section  research evidence shall be presented which supports this original hypothesis.


The placebo effect has resisted prediction in experimental work for over 50 years. Many psychological variables have been tested to see if they can predict the response, but all have failed \cite{Shapiro1997} and it has been argued that placebo responders do not even exist \cite{Kaptchuk2008a}. The thesis of this research paper is that the lack of results in the prediction of the placebo effect is an artifact of the methods used rather than the unpredictability of the effect. This research argues that the Implicit Association Test \cite{Greenwald1998} is a useful method with which to predict the response. 
...
The placebo is a badly defined but widely used concept in medicine \cite{Kaptchuk1998,Macedo2003} . Many theories have been put forth to account for its effects, including expectancy, conditioning, emotional change, motivation and meaning \cite{Stewart-Williams2004b,Moerman2006,Vodouris1989,Geers2005a}. However, none of these theories can account for all of the effects observed in research. The conditioning approach gained a following at first \cite{Voudouris1985}  but was quickly opposed by the response expectancy theory of Kirsch \cite{Kirsch1985,Kirsch1997}. Research on these two competing perspectives lead to a number of findings which may help us elucidate some of the features of this strange effect. In this section, the  expectancy versus conditioning debate shall be reviewed insofar as it relates to the main point, which is to examine whether or not the use of implicit measures of expectancies will allow us to predict the placebo response in individuals with greater accuracy. Then  some other lines of evidence which support this case will be reviewed, and finally sum up the evidence in favour of my hypothesis.
...
This conclusion is further reinforced by the work of Shiv and Carmon \cite{Shiv2005a} on the placebo effects of energy drinks given to participants at a lowered price. This research used an outcome measure of the number of problems solved in a specified period, and it demonstrated that those who believed that the price had been discounted solved less puzzles than those who received the drink at the normal price. The interesting feature of this research (for the purposes of this paper) is that when participants attention was drawn to the discounted price, the difference between the groups was reduced. This would seem to argue in favour of an implicit learning situation in this experiment also. 
...
All participants volunteered to take part in the study, as an incentive a draw was held where one randomly selected participant won a smartphone. The study was approved by the Social Research Ethics Committee of the lead researcher's university. 
...
\item A manual blood pressure gauge and hand exerciser used to induce pain via the submaximal torniuqet technique. 
...
The priming manipulation appeared to achieve its intended purpose, as Figure \ref{fig:placprimeplot} shows. A logistic regression demonstrated that the effects of priming were significant (p=0.0452). 

Next, the potential contamination of the IAT results by general processing speed was assessed. 
...
randf.train <- train(PlacResp~., data=iatandexpfull.train, method="rf", trControl=mytrain, tuneGrid=data.frame(.mtry=1:10), preProcess=c("center", "scale"))
...
<<gbmtrain, echo=FALSE, results=hide>>=
## gbm.train <- train(PlacResp~., data=iatandexpfull.train, method="gbm", trControl=mytrain
##                    , tuneGrid=data.frame(.shrinkage=(c(0.1, 0.01, 0.001)), .n.trees=c(50,100,150), .interaction.depth=1))
@ 
...
glmboost.train <- train(PlacResp~., data=iatandexpfull.train, method="glmboost", trControl=mytrain, tuneGrid=data.frame(.mstop=1:100, .prune=1:100))
@ 
...
glmnet.train <- train(PlacResp~., data=iatandexpfull.train, method="glmnet", trControl=mytrain, tuneGrid=data.frame(.alpha=seq(0.1, 1, by=0.1), .lambda=seq(0.1, 1.0, by=0.1)))
@ 
...
rpart.train <- train(PlacResp~., data=iatandexpfull.train, method="rpart", trControl=mytrain, tuneGrid=data.frame(.cp=seq(0.01, 1, by=0.01)))
@ 
...
ctree2.train <- train(PlacResp~., data=iatandexpfull.train, method="ctree2", trControl=mytrain, tuneGrid=data.frame(.maxdepth=seq(0,10, by=1)), preProcess=c("center", "scale"))
@ 

<<evtree, echo=FALSE, results=hide>>=
evtree.train <- train(PlacResp~., data=iatandexpfull.train, method="evtree", trControl=mytrain, tuneGrid=data.frame(.alpha=1:10), preProcess=c("center", "scale"))
@ 

<<cforest, echo=FALSE, results=hide>>=
cforest.train <- train(PlacResp~., data=iatandexpfull.train, method="cforest", trControl=mytrain, tuneGrid=data.frame(.mtry=seq(1, 101, by=10)) )
@ 
...
randf.acc <-confusionMatrix(randf.pred, iatandexpfull.test[,"PlacResp"])
nb.acc <-confusionMatrix(nb.pred, iatandexpfull.test[,"PlacResp"])
...
glmboost.acc <- confusionMatrix(glmboost.pred, iatandexpfull.test[,"PlacResp"])
glmnet.acc <- confusionMatrix(glmnet.pred, iatandexpfull.test[,"PlacResp"])
...
rpart.pred <- confusionMatrix(rpart.pred, iatandexpfull.test[,"PlacResp"])
