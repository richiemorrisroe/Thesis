...
gend.cond <- print(xtable(with(Iatandexpmeasures.phys, table(Gender, Condition)), caption="Gender Breakdown by Condition", label="tab:gendcond"))
@ 

Table \ref{tab:gendcond} shows the breakdown of participants' gender by their condition. 

<<gendprime, echo=FALSE, results=tex>>=
prime.gend <- print(xtable(with(Iatandexpmeasures.phys, table(Gender, Prime)), caption="Gender Breakdown by Priming Condition", label="tab:primegend"))
@ 
...
\begin{figure}
<<corriattcqopt, echo=FALSE, figure=TRUE, pdf=TRUE, eps=TRUE, png=TRUE>>=
corr.iat.tcq.opt.pl <- plotmatrix(iat.block.merge2[,2:length(iat.block.merge2)])+geom_smooth(method="lm")
print(corr.iat.tcq.opt.pl)
@   
...
\begin{figure}
<<placprimeplot, echo=FALSE, figure=TRUE, pdf=TRUE, png=TRUE>>=
plac.prime.pl <- ggplot(Iatandexpmeasures, aes(x=PlacResp))+geom_histogram()+facet_grid(.~Prime)
print(plac.prime.pl)
@   
...
\begin{figure}
<<ggplothistcond, echo=FALSE, fig=TRUE, eps=TRUE, pdf=TRUE, png=TRUE>>=
...
\begin{figure}
<<optiatcorrsurv, echo=FALSE, figure=TRUE, eps=TRUE, pdf=TRUE, png=TRUE>>=
optiat.histcond <- ggplot(Iatandexpmeasures, aes(x=OptIAT.Mean))+geom_density()+facet_grid(.~PlacResp)
print(optiat.histcond)
@   
...
\begin{figure}
<<ggplotplacyesno, echo=FALSE, fig=TRUE, eps=TRUE, pdf=TRUE, png=TRUE>>=
...
require(doMC)
registerDoMC(2)
iatandexpfull <- Iatandexpmeasures.phys[,c("PlacResp", "Age", "Gender", "Prime", "LOTR", "MAAS", "Pill", "Cream", "Inj", "Acu", "Hom", "Rei", "OptIAT.Mean", "OptIAT.Median", "TCQIAT.Mean", "TCQIAT.Median", "meanconv", "meanalt", "convaltcomp", "gsr.mean")]
