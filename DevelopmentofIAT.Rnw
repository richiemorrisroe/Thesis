...
setwd("ExperimentDataforR/Pilot")
source("../../func.R")
optiat <-read.table("Opt IAT Pilot data 191110 FINISHED.csv", header=TRUE, sep="\t" )
...
iepilot<-read.csv("I and E for Pilot.csv")
require(psych)
require(xtable)
@

<<scalecreation, echo=FALSE, results=hide>>=
scales<-cbind(iepilot$LOTR, iepilot$MAAS, iepilot$Pill, iepilot$Cream, iepilot$Inj, iepilot$Acu, iepilot$Hom, iepilot$Rei)
optpesswords<-cbind(iepilot$Disimproving, iepilot$Worse, iepilot$Sad, iepilot$Failing, iepilot$Happy, iepilot$Succeeding, iepilot$Better, iepilot$Improving)
@
...
xtable(summary(iepilot$Gender))	#$
@

<<age, echo=FALSE, results=tex>>=
xtable(summary(iepilot$Age))	#$
@

<<iatscores, echo=FALSE, results=tex>>=
IAT<-cbind(iepilot$DIAT.Conv, iepilot$DiatOPt)
print(xtable(cor.test(IAT, use="complete.obs", method="spearman")))
@

<<maascorr, echo=FALSE, results=tex>>=
print(xtable(cor.test<-(IAT, iepilot$MAAS, use="complete.obs", method="spearman"))) #$
@

<<srcorr, echo=FALSE, results=tex>>=
print(xtable(cor.test<-(iepilot$IATOpt, iepilot$LOTR, use="complete.obs", method="spearman"))) #$
@
