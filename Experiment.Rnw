...
surv.acu.pl <- ggplot(tcq2, aes(x=Acutot))+geom_density()
...
<<alttreatmentssurvey, echo=FALSE, fig=TRUE,pdf=TRUE, eps=TRUE, png=TRUE>>=
surv.hom.pl <- ggplot(tcq2, aes(x=Homtot))+geom_density()
...
<<alttreatmentssurvey, echo=FALSE, fig=TRUE,pdf=TRUE, eps=TRUE, png=TRUE>>=
surv.rei.pl <- ggplot(tcq2, aes(x=Reitot))+geom_density()
...
The results of a chi-square test showed that the gender of participants in each condition were equivalent, (p=\Sexpr{chisq.gend[["p.value"]]}).
