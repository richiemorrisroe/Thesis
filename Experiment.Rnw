...
expfull[,"Participant"] <- with(expfull, sprintf("%04d", Participant))
...
vasscores[,"PPNo."] <- with(vasscores, sprintf("%04d", PPNo.))
...
tcq.xtab <- xtable(summary(aov(TCQIAT.Mean~Condition, data=Iatandexpmeasures)), label="tab:tcqiatcheck", caption="Summary of ANOVA of Treatment Credibility IAT Scores by Condition")
...
optiat.first.aov <- summary(aov(OptIAT.Mean~first_IAT, data=Iatandexpmeasures.firstiat))
tcqiat.first.aov <- summary(aov(TCQIAT.Mean~first_IAT, data=Iatandexpmeasures.firstiat))
@
...
Iatandexpmeasures.top <- merge(Iatandexpmeasures, gsr.top.av2, by="Participant")
...
Iatandexpmeasures.phys <- merge(Iatandexpmeasures, gsr.mean, by="Participant")
...
iatandexp.s <- Iatandexpmeasures[,c(14:22, 24)]
...
placmod.full <- glm(PlacResp~TCQIAT.Mean*OptIAT.Mean*LOTR+Age+Gender+Prime+MAAS+Pill+Cream+Inj+Acu+Hom+Rei+convaltcomp, data=iatandexpfull.train, family=binomial(link="logit"))
...
iatandexpfull.test.small <- iatandexpfull.test[, c("PlacResp", "TCQIAT.Mean", "LOTR", "Age", "OptIAT.Mean", "Rei")]
...
placmod.test <- glm(PlacResp~TCQIAT.Mean*LOTR+Age+OptIAT.Mean+Rei,data=iatandexpfull.test.small, family=binomial(link="logit"))
...
placglm <- function (data, indices) {
    d <- data[indices,]
    mod <- glm(PlacResp~TCQIAT.Mean*LOTR+Age+OptIAT.Mean+Rei,data=d, family=binomial(link="logit"))
...
exp.sem <- Iatandexpmeasures.t[, c("Age", "Gender", "LOTR", "OptIATMean", "TCQIATMean", "PlacResp", "meanconv", "meanalt", "convaltcomp", "Prime","gsrmean" )]
@ 
...
gsr.cond <- lm(gsr.mean~Condition, data=Iatandexpmeasures.phys)
