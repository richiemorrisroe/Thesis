...
gsr.gend.m2 <- gsr.gend.m[with(gsr.gend.m, index<=2000),]
...
gsr.placresp.m2 <- gsr.placresp.m[with(gsr.placresp.m, index<=2000),]
...
gsr.conditions.m2 <- gsr.conditions.m[with(gsr.conditions.m, index<=2000),]
...
cond.m2 <- cond.m[with(cond.m, index<=2000),]
...
rownames(placmod.fit.all) <- c( "Optimism", "Credibility", "Kirsch Two Factor")
print(xtable(placmod.fit.all, label="tab:placmodsemfit", caption="Fit Measures for Placebo Response Structural Equation Models"), scalebox=0.6)
@ 
