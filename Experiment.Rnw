...
Finally, the models described in Chapter \ref{cha:methodology} and in Section \ref{sec:introduction-1} were fit to the data. These models allowed for the principled comparision of a number of different theoretical models on the relationship between expectancies, optimism and physiological variables on the placebo response. 
...
@   
  \caption{Scatterplot and Correlelagram for Implicit and Explicit Measures}
