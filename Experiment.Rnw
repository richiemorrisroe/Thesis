...
\item The results of Kirsch (1998) showed that informed conditioning was less effective than uninformed conditioning would suggest that the effectiveness of placebos can be reduced by conscious processing;
...
The method variance problem was moderated by the use of the D measure from Greenwald \cite{Greenwald2003}, as demonstrated by Mierke \cite{Klauer2005}. The impact of these potential confounders was examined by assessing the correlations between the two IAT measures used in this study (Treatment Credibility and Optimism) and if these were two high ($r\ge 0.3$) then controlling for them by including both IAT's in each regression model. 
...
The pain data was also used to classify participants as either placebo responders or non-responders. This was done simply by examining if their pain levels decreased following administration of placebo. If this happened, they were classified as placebo responders, and if not, they were classified as non-responders. %% Another approach taken was to examine the number of minutes spent with a pain rating of less than seven following administration of the placebo, and to model this as a Poisson variable using a generalised linear model. 
...
tcqiat.pp.time <- select(tcqiat, Participant, Date, Time)
names(tcqiat.pp.time)[3] <- "Time_TCQ"
tcqiat.pp.time[,"Time_TCQ"] <- with(tcqiat.pp.time, hms(Time_TCQ))
optiat.pp.time <- select(optiat, Participant, Date, Time)
...
iat.first.pp <- select(iat.times.merged.distinct, Participant, first_IAT)
...
print(xtable(gsr.start.cond.sd, label="tab:gsrstartcond", caption="Mean and SD of GSR by Condition at baseline (first 100 seconds)"))
@ 
...
As can be seen above in Table \ref{tab:placmodstep}, the model chosen by the stepwise selection procedure has an almost significant coefficient on the TCQ IAT, one for the LOT-R,  and the two two-way interactions trended towards significance. 
...
print(xtable(placmod.fit.all, label="tab:placmodsemfit", caption="Fit Measures for Placebo Response Structural Equation Models"), scalebox=0.8)
@ 
