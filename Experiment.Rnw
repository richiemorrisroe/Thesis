...
\subsection{Using Psychometric Models from Survey Samples on the Experimental Participants}
...
<<grm1pltcqsmall, echo=FALSE, results=hide, eval=FALSE>>=
...
Iatandexpmeasures[,"Participant"] <- with(Iatandexpmeasures, sprintf("%04d", Participant))
...
@ 
\begin{figure}
<<gsrplacresplot, echo=FALSE, fig=TRUE>>=
...
<<mod1sem, echo=FALSE, results=hide>>=
Iatandexpmeasures2 <- Iatandexpmeasures
names(Iatandexpmeasures2)[c(21,23)] <- c("OptIATMean", "TCQIATMean")
exp.sem <- Iatandexpmeasures2[, c("Age", "Gender", "LOTR", "OptIATMean", "TCQIATMean", "PlacResp", "convaltcomp")]
...
<<placmod1semrun, echo=FALSE, results=hide>>=
