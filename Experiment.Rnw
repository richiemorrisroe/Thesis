...
The measure developed in Chapter~\ref{cha:tcq-thesis} attempts to address the first problem, while the IAT's developed in Chapter~\ref{devel-impl-meas} address the second. 
...
Table \ref{tab:placmodsteptest} shows the results of the stepwise selected model on the test data. It can be seen that none of the variables are significant, which is not surprising given the sample size. The odds ratios are a little more infomative. These are shown in  Table~\ref{tab:placmodstepodds}. As can be seen, there were no significant differences from zero in this sample, suggesting that if there is an effect here, the sample is not large enough to uncover it, and as such the effect must be a small one (as this experiment was powered to uncover only medium or large effects). 
...
\subsection{Theoretical Models of the Relationship between Placebo Response and Implicit and Explicit Measures}
