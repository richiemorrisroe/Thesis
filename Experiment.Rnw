...
surv.exp.mind.pl <- ggplot()+geom_density(data=hom.data, aes(x=mindfulness), fill="blue", alpha=0.5)+geom_density(data=Iatandexpmeasures, aes(x=MAAS), fill="red", alpha=0.5)
surv.exp.opt.pl <- ggplot()+geom_density(data=hom.data, aes(x=optimism), fill="red", alpha=0.5)+geom_density(data=expmeasures2, aes(x=LOTR), fill="blue", alpha=0.5)
...
As can be seen above in Table \ref{tab:placmodstep}, the model chosen by the stepwise selection procedure has a significant coefficient on the TCQ IAT, one for Age and significant coefficient for the interaction between the TCQIAT and the LOTR (while the individual coefficient for the LOT-R trended towards significance). 
...
<<ecgimport, echo=FALSE, results=hide>>=
