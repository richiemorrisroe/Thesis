...
print(plotmatrix(tcqstimblock3)+geom_smooth())
@  
...
print(plotmatrix(tcqstimblock5)+geom_smooth())
@ 
...
## optblocks3and5 <-  merge(optblock3, optblock5, by="part.opt")
#this whole chunk needs to be fixed, refactor into a function use mapply to reduce the need for laborious repitition
...
evtree.train <- train(PlacResp~OptIAT.Mean+TCQIAT.Mean+meanconvo, data=iatandexpfull.train, method="evtree", preProcess=c("center", "scale"))
...
\begin{figure}
<<evtreeplot, echo=FALSE, figure=TRUE>>=
...
names(gsr.mat) <- NULL
names(ecg.mat) <- NULL
gsr.df <- as.data.frame(gsr.mat)
names(gsr.df) <- gsrnames
ecg.df <- as.data.frame(ecg.mat)
names(ecg.df) <- ecgnames

@


<<GSRcollate, echo=FALSE, results=hide>>=
gsrdata <- phys.df[,with(phys.df, grep("GSR", x=names(phys.df)))]
