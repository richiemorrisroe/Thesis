...
However, there was a difference in the mean (and median) GSR scores at baseline. Baseline measurements were taken for the first one hundred seconds of the experiment, and can be seen in Table \ref{tab:gsrstartcond} broken down by condition. The reasons for this difference are unclear, given that no participant was informed of their condition allocation until after the bandage had been applied.  

<<gsrcondstart, echo=FALSE, results=tex>>=
print(xtable(gsr.start.by.cond, label="tab:gsrstartcond", caption="Mean GSR by Condition at baseline (first 100 seconds)"))
@ 
...
@ 
<<fascoresmaaslotr, echo=FALSE, results=hide>>=
