...
Another issue with current placebo research relates to expectancies. The first issue is that expectancies are typically measured with a simple one question scale. One of the aims of this thesis was to apply psychometric methods to the development of a more sophisticated measure of treatment credibility and expectancies (see Chapter \ref{cha:tcq-thesis}). The other problem is that expectancies are assumed to be conscious, even though they appear to have far more in common with unconscious responses than they do with controlled processes. 
...
Recruitment for participants primarily took place via email and social networking sites. In total, six emails were sent to random sub-sets of the student mailing list from the period January 2nd until April 11th. 
...
@   
  \caption{Density Plot for Distribution of Pill Credibility Scores, Survey Sample (sample 2) (top) and Experimental Cream Credibility Totals (bottom)}
  \label{fig:compcream}
\end{figure}
...
\begin{figure}
<<corrtestimplexpl, echo=FALSE, results=hide>>=
...
. Another surprise was that the direction of the correlation between the LOT-R and the MAAS was opposite to that observed in prior research. Possible reasons for these results are considered in the Discussion, and some theories were described in Chapter \ref{cha:health-for-thesis}. 
...
Using a chi-square test ($\chi^2(1)=0.32, p=0.56$) there was no significant difference between the Deceptive and Open Placebo conditions on the probability of placebo response, which would suggest that this experiment was not successful at inducing a response reliably. This is damaging, but not devastating to the main point of the research.  
...
print(xtable(summary(placmod.step), label="tab:placmodstep", caption="Model selected by Stepwise Selection over the training data"), table.placement="htp")
@ 
...
print(xtable(summary(placmod.test), label="tab:placmodsteptest", caption="Coefficients of Stepwise Selected Predictors on Test Data"), floating=FALSE)
...
print(xtable(odds.ratios, label="tab:placmodstepodds", caption="Odds Ratios for Coefficients in Test Data Model"), floating=FALSE)
@
...
print(xtable(pvalues.sum, label="tab:placmodbootpvalues", caption="Summary statistics for the bootstrapped p-values"), floating=FALSE)
@ 
...
print(placmod2.xtab,  floating=FALSE)
@ 
...
print(placmod.lotr.xtab, floating=FALSE)
@ 
...
print(xtable(summary(placmod.lotr.irt), caption="Regression of Placebo Response on IRT Estimated Optimism Scores", label="tab:placmodlotrirt"), floating=FALSE)
@ 
...
This section was perhaps the most interesting, in that some observations were made that do not appear to have been reported previously in the literature. 
