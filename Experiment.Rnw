...
\item The results of Kirsch (1998) where he showed that informed conditioning was less effective than uninformed condition would seem to argue that the effectiveness of placebos can be reduced by information;

\item Some theorists have suggested that the IAT captures substantial state variance (as evidenced by low test-retest reliability) and it has also been argued that response to placebo is similar, thus suggesting that there may be a link;
...
The implicit measure chosen for this purpose was the implicit association test. This choice was made both because it is the most widespread, and as such more of the best practices and problems with the measure are available in prior research. 
...
However, this study did not include a deceptive placebo condition, and so is not a true test of the theory that deception is unnecessary for the placebo response. One secondary aim of this study was to examine the theory that Open Placebo responses could be induced in a placebo analgesia paradigm. 
...
Skin conductance recordings (GSR) were taken as part of this experiment. This source provided recordings at 1000Hz for the entire procedure (and normally a few seconds after the blood pressure gauge). %% The ECG data was analysed to extract heart rate, heart variability  and QT and RR intervals.
...
\end{figure}
 ...
 %% \begin{figure}
<<tcqmeanresp, echo=FALSE,pdf=TRUE, eps=TRUE, png=TRUE, eval=FALSE>>=
...
The results of the Chi-Square test( $\chi^2(11)=64.0, p\le 0.0001$) show that priming appears to be the driver of this effect, as the Placebo Response by Condition is extremely significant given the priming manipulation. Most of this improvement occurred in the Open Placebo group, which is an extremely interesting finding, one not reported in the literature before.  
...
\begin{figure}
<<medianpaintscorrelationplots, echo=FALSE, fig=TRUE,pdf=TRUE, eps=TRUE, png=TRUE, results=hide>>=
dev.new()
par(mfrow=c(2,2))
...
\begin{figure}
<<meanpaintscorrelationplots, echo=FALSE, fig=TRUE,pdf=TRUE, eps=TRUE, png=TRUE, results=hide>>=
plot.new()
par (mfrow=c (2,2))
sink ("tmp.txt")
...
@ 
\begin{figure}
<<gsrpainplt, echo=FALSE, fig=TRUE, eps=TRUE, png=TRUE, pdf=TRUE, jpg=TRUE>>=
...
expimpall <- rbind(expimp1.fit.meas, twofac.fit.meas, expimpsep.fit.meas, expimpmoderated.fit.meas, expimp.nocorr.fit.meas)
rownames(expimpall) <- c("Direct", "Two Factor", "Moderated", "Credibility/Optimism", "Uncorrelated")
