...
rf.train <- train(PlacResp~., data=iatandexpfull)
rf.pred <- predict(rf.train, iatandexpfull.test)
test.xtab <- xtable(confusionMatrix(rf.pred, iatandexpfull.test$PlacResp)$table)
...
tcqiatgender <- ggplot(Iatandexpmeasures, aes(x=Gender, y=TCQIAT.Mean))
...
optiatgender <- ggplot(Iatandexpmeasures, aes(x=Gender, y=OptIAT.Mean))
...
surviat.corr <- ggplot(Iatandexpmeasures, aes(x=lengthsurv, y=TCQIAT.Mean, colour=Condition))
surviat.comp <- surviat.corr+layer(geom="smooth")
print(surviat.comp)
@ 

<<placresponse.surv.tcqiatscore, echo=FALSE, fig=TRUE>>=
surviat.corr2 <- ggplot(Iatandexpmeasures, aes(x=lengthsurv, y=TCQIAT.Mean, colour=PlacResp ))
...
optiat.corr <- ggplot(Iatandexpmeasures, aes(x=lengthsurv, y=OptIAT.Mean, colour=PlacResp))
