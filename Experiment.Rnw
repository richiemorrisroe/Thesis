...
corr.imp.exp <- corr.test(Iatandexpmeasures[,14:25], method="kendall")
...
tcq.xtab <- xtable(summary(aov(tcqiatscore~Condition, data=Iatandexpmeasures)), label="tab:tcqiatcheck", caption="Summary of Anova of Treatment Credibility Scores by Condition")
opt.xtab <- xtable(summary(aov(optiatscore~Condition, data=Iatandexpmeasures)), label="tab:optiatcheck", caption="Summary of ANOVA of Optimism IAT scores by Condition")
...
Next, the differences between the experimental sample and the population sample were examined. 
\begin{figure}
<<surveyoptimism, echo=FALSE, figure=TRUE>>=
survey.opt.pl <- ggplot(hom, aes(x=optimism))+geom_histogram()
print(survey.opt.pl)
@   
...
This finding is much more clearly conveyed in Figure \ref{fig:placprimeplot}, where it can be seen that Participants were much more likely to respond to placebo following a priming intervention. 
