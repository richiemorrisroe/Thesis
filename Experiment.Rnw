...
print(xtable(pvalues.sum, label="tab:placmodbootpvalues", caption="Summary statistics for the bootstrapped p-values"))
@ 
...
print(xtable(survtable.cond, label="tab:survlengthcondplac", caption=" Median Recording Time (secs) by Condition and Placebo Response. Time is counted from the beginning of physiological monitoring (approximately 5 minutes before application of painful stimuli"))
@ 
...
The next step in the use of SEM models was to examine whether a two factor or one factor model best fitted the relationship between implicit and explicit models. Comparing a model where both implicit and explicit measures loaded on one factor to a model in which they loaded on separate factors showed that the two factor solution provided a better fit to the data. Given the lack of data for a test sample in this portion of research, these results should be regarded as provisional until replicated. 
