...
napainratings2 <- data.frame(Participant=vasscores$PPNo., napainratings)
...
surv.pill.pl <- ggplot(+geom_density()
...
surv.exp.cream.pl <- ggplot()+geom_density(data=credtotals, aes(x=Creamtot), fill="red")+geom_density(Iatandexpmeasures, aes(x=Cream), fill="blue")
...
painratings[,"Participant"] <- with(painratings, sprintf("%04d", as.numeric(PPNo.)))
