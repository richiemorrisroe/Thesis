...
surviat.corr <- ggplot(Iatandexpmeasures, aes(x=lengthsurv, y=TCQIAT.Mean, colour=Condition))+geom_smooth(span=0.3)
print(surviat.corr)
@   
  \caption{TCQIAT.Mean against Pain Responses Over Time}
  \label{fig:tcqpaintime}
\end{figure}


<<placresponse.surv.tcqiatscore, echo=FALSE, fig=TRUE>>=
surviat.corr2 <- ggplot(Iatandexpmeasures, aes(x=lengthsurv, y=TCQIAT.Mean, colour=PlacResp ))+geom_smooth(span=0.6)
print(surviat2.comp)
@ 
