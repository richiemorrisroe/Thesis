...
@   
  \caption{Density Plot for Distribution of Pill Credibility Scores, Survey Sample (sample 2) versus experimental sample (top) and Cream Credibility Totals (bottom) sample 2 survey}
  \label{fig:compcream}
\end{figure}
...
pairs.panels(Iatandexpmeasures[,14:25])
...
@   
  \caption{Scatter-plot and Correlelagram for Implicit and Explicit Measures}
