...
placmod.cred.opt.mod <- lavaanify(placmod.cred.opt, std.lv=TRUE)
...
placmod.first.fit <- sem(placmod.first.mod, data=na.omit(exp.sem), orthogonal=TRUE)
...
Finally, it was determined that neither implicit or explicit expectancies alone could mediate the effects of placebo, but the two of them together showed some promise as a method to predict the placebo response. It really needs to be clarified that given the amount of leeway taken with the modeling process in this research that these findings must be regarded as tentative until borne out by future research. 
