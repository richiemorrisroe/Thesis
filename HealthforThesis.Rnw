...
Nex, the summary statistics for the second sample (N=1107) were examined and reported. 
...
The first factor appears to contain all of the scales except for Physical Functioning, which loads on Factor 2. We can best term this factor as General and Emotional Health.
...
print(xtable(coef(lotr2c.grm.1pl), caption="Coefficient Estimates for LOT-R One Parameter Graded Response Model, Split C", label="tab:lotr2cgrm1pl"))
@ 
...
cors <- scale.cor$r
print(xtable(as.data.frame(cors), label="tab:hom2scalecor", caption="Spearman Correlations between Scale Totals across both Samples" ), scalebox=0.5)
@ 
...
\subsection{Regression Analyses, Sample One}
...
print(xtable(summary(health.step.test), caption="Coefficients for General Health Regression Model with predictors chosen by Stepwise Selection on unseen data, Sample One", label="tab:hom1healthsteptest"))
@ 
As can be seen from Table \ref{tab:hom1healthsteptest}, optimism, role limitations and energy fatigue were retained as predictors in the model. None of the variables were significant, with the exception of energy/fatigue. 
...
\section{Regression Analyses, Sample Two}
...
just one factor.  A   replicable and parsimonous 8 factor structure for the RAND MOS was demonstrated, 
