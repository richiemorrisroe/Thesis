...
@
  \caption{Density Plot of Missing values in paper sample. x Axis is a count of the number of missing observations in each variable }
...
<<opttrain, echo=FALSE, results=hide>>=
  ## opt.trainenet <- train(optimism~generalhealth+Age+College+pain+mindfulness+socialfunctioning+rolelim+rolelimem+emwellbeing+physfun+energyfat, data=hom1.train.opt, method="enet", tuneLength=5, fitBest=FALSE, returnData=TRUE)
...
## opt.train.rf <- train(optimism~generalhealth+pain+mindfulness+socialfunctioning+rolelim+rolelimem+emwellbeing+physfun+energyfat, data=hom1.train.opt, method="rf", tuneLength=5)
## opt.train.evtree <- train(optimism~generalhealth+pain+mindfulness+socialfunctioning+rolelim+rolelimem+emwellbeing+physfun+energyfat, data=hom1.train.opt, method="evtree", tuneLength=5)
trainopt <- with(hom1.train.opt, optimism)
trainpred <- hom1.train.opt[,c(4, 66:74)]
testopt <- with(hom1.test.opt, optimism)
## optpredenet <- extractPrediction(list(opt.trainenet), testX=testpred, testY=testopt, verbose=TRUE)
## opt.pred.train.enet <- extractPrediction(list(opt.trainenet))
## opt.pred.test <- extractPrediction(list( opt.train.rf, opt.trainenet), testX=hom1.test.opt[,-75], testY=testopt)
@ 
...
opt.ridge.pred <- penalisedRegression(x=trainpred, y=trainopt, newy=testopt, traindata=hom1.train.opt, testdata=testpred, alpha=0, nfolds=10, type="response", scale=FALSE)
...
opt.lasso.pred <- penalisedRegression(x=trainpred, y=trainopt, newy=testopt, traindata=hom1.train.opt, testdata=as.matrix(testpred), alpha=1, nfolds=10, type="response", scale=FALSE)
...
print(FactorXtab(rand.fact.13,label="tab:tcq1rand13fact", caption="Factor Loadings, Thirteen Factor Solution RAND MOS, Sample One (oblimin rotation)"))
@
...
<<hom1physlimgrm2pl, echo=FALSE, results=hide>>=
...
As can be seen from Table \ref{tab:homoptridge2a} mindfulness again appears to be the most useful predictor of optimism, and in general the coefficients are quite similar to those from Study One.

Finally, the model developed by stepwise selection was applied to the new data. 

<<optstep2a, echo=FALSE, results=hide>>=
opt.step.test <- lm(optimism~generalhealth+Age+emwellbeing, data=scales2a.full)
