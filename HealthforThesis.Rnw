...
The majority of the analytical procedures were as described in Chapter \ref{cha:methodology}. In brief, Factor analytic and item response theory (not Rasch) models were fit to the data, and tested on either new samples (Sample One) or on other splits of the data (Sample Two). All regression results were obtained through the use of ten-fold cross-validation on the training set, and the reported coefficients are from the test set, and thus are unbiased by the search process. In general, all correlations reported are rank-correlations, to control for the non-normality of the input variables. 
...
rand8.cor[lower.tri(rand8.cor, diag=TRUE)] <- NA
...
print(xtable(mxCompare(base=maas.bt.full.fit, comp=c(maas.bt.red1.fit, maas.bt.reduced2.fit)), label="tab:maasbacktestsemcompare", caption="Full and Reduced Models for MAAS Tested on Sample One Data"), scalebox=0.85)
@ 
...
print(xtable(mxCompare(lotr.bt.small.fit, lotr.bt.full.fit), label="tab:lotrbtsemcompare", caption="Performance of Full and Reduced Model for LOT-R on Sample One Data"), scalebox=0.85)
...
cors.xtab[lower.tri(cors.xtab, diag=TRUE)] <- NA
