...
<<randirtscales, echo=FALSE, results=hide>>=
irtphysfun2b <- randitems2[,paste(rand, c(3:12), sep="")]
irtneghealth2b <- randitems2[,paste(rand, c(17:19, 24:25,28,29,31,32,35 ), sep="")]
irtposhealth2b <- randitems2[,paste(rand, c(1, 20, 23, 26,27,30, 34,36), sep="")]
@
...
As can be seen from Table \ref{tab:randneghealthitemord},quite a number of items failed the item ordering assumption (RAND1, RAND18, RAND19, RAND35, RAND26) and so were removed from the scale before further analysis. 

<<newIrtEmHealth, echo=FALSE, results=tex>>=
irtneghealth2b.s <- irtneghealth2b[,paste(rand, c(25,29,31,32), sep="")]
