...
<<importdata, echo=FALSE, results=hide, cache=TRUE>>=
...
<<loadpackages, echo=FALSE, results=hide, cache=TRUE>>=
require(ggplot2)
require(psych)
require(xtable)
require(OpenMx)
...
<<sourcefunc, echo=FALSE, results=hide, cache=TRUE>>=
source("func.R")
@
...
<<loadirtpackages, echo=FALSE, results=hide, cache=TRUE>>=
require(mokken)
require(eRm)
require(ltm)
@
...
\item A model where the effect of health and optimism is mediated through the other predictor variables;
...
mxPath(from="generalhealth", to="emwellbeing", free=TRUE, values=0.5),
mxPath(from="generalhealth", to="energyfat", free=TRUE, values=0.5),
                            mxPath(from="generalhealth", to="energyfat", free=TRUE, values=0.5),
                            mxPath(from="generalhealth", to="Age", free=TRUE, values=0.5),
                            mxPath(from="emwellbeing", to="energyfat", free=TRUE, values=0.5),
                            mxPath(from="emwellbeing", to="Age", free=TRUE, values=0.5),
                            mxPath(from="one", to=variables, arrows=1),
                            mxPath(from=predictors, to=response))
opt.direct.fit <- mxRun(model=lotr.rand.direct, intervals=TRUE)
@ 
