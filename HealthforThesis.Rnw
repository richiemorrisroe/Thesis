...
setOption("xtable.include.rownames"=FALSE)
...
print(tot.xtab, scalebox=0.75, include.rownames=FALSE, floating.environment="sidewaystable")
@
...
print(hom.tot.cor.xtab, scalebox=0.6, floating.environment="sidewaystable")
@
...
optplotcoll <- ggplot(na.omit(hom1), aes(x=generalhealth, y=optimism))+layer(geom="smooth", method="lm")+facet_grid(~.College, ncol=2)
print(optplotcoll)
@  
...
print(FactorXtab(rand.fact.13,label="tab:tcq1rand13fact", caption="Factor Loadings, Thirteen Factor Solution RAND MOS, Sample One (oblimin rotation)"), scalebox=0.5)
@
