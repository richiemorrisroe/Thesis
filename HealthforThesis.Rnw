...
~\cite{Carson2004}, but mindfulness and optimism have not tended to be researched together. 
...
The majority of the first sample (N=392) were undergraduate students (N=304), and the mean age in the sample was 23.18 (SD=7.9). The majority of the second sample (N=1107) were undergraduate students (N=937) while the mean age was 22.3 (SD=6.8). 
...
Next, the summary statistics for the second sample (N=1107) were examined and reported. 
...
The parallel analysis criterion and the kaiser criterion suggest eight factors. The acceleration factor and the optimal calibration index measures suggested two factors. These two measures may be picking up on the higher order factor structure of the items, as the RAND is typically modelled as having two higher order factors (physical and mental health).
...
<<rand2fact, echo=FALSE, results=hide, eval=FALSE>>=
...
@
The results of the two factor solution (Appendix, Page \pageref{tab:rand2fact}) were as follows:
...
print(rand8.xtab, scalebox=0.7, digits=2, sanitize.text.function= function (x) x)
@
...
rand8corr.xtab <- xtable(rand.fact.8[["r.scores"]], label="tab:hom1rand8corr", caption="Factor Correlations, Eight Factor Solution RAND MOS, Sample One")
...
print(xtable(randsemcomp.hom2,label="tab:randsemcomparehom2", caption="Model Comparison for RAND MOS data using Models from Sample 1 on Sample 2" ), scalebox=0.8)
@
...
print(xtable(maasquestions, label="tab:maasitems", caption="MAAS Item Content"), include.rownames=FALSE, scalebox=0.7)
@ 

Table~\ref{tab:maasitems} shows the item content. 
...
The next factor solution to be examined was the three factor solution (loadings shown in Appendix, \pageref{
...
@
This five factor solution for Split B (Appendix, page \pageref{tab:tcq2bmaasfact5}) explained 54\% of the variance in the sample.
...
print(xtable(maas2b.semcomp,label="tab:maas2bsemcompare", caption="Comparison of Factor Structures for MAAS 2B Solutions, Tested on Split B"), scalebox=0.9)
@
...
print(xtable(maas2c.semcomp,label="tab:maas2csemcompare", caption="Comparison of Factor Structures for MAAS 2C Solutions, Tested on Split C"), scalebox=0.9)
@
...
The Mokken analysis suggests that two items should be dropped from the scale, items 2 and 6. This leaves a thirteen item scale for further analysis. Note that items 1 and 6 has extremely low communalities in Table~\ref{tab:maasfact1average}, which reinforces the decision to remove these items from the scale. 

There were no violations of the monotonicity assumption for the reduced scale. The item coefficients (ItemH) are quite low, many of them hang around 0.30, which is the minimum allowed. An item ordering analysis suggested that Q2 and Q6 should be removed from the scale before analysis was carried out, and this was done.
...
maas.grm.1pl.coef <- coef(maas.grm.1pl)

print(xtable(coefirt(maas.grm.1pl), caption="Coefficient Estimates for MAAS One Parameter Graded Response Model, Sample One", label="tab:maasgrm1pl"), sanitize.colnames.function=function (x) print(x))
@ 
...
print(xtable(coefirt(maas2b.grm.1pl), caption="Coefficient Estimates for MAAS One Parameter Graded Response Model, Split B", label="tab:maas2bgrm1pl"), sanitize.colnames.function=function (x) print(x))
@ 
...
print(xtable(coefirt(maas2c.grm.1pl), caption="Coefficient Estimates for MAAS One Parameter Graded Response Model, Split C", label="tab:maas2cgrm1pl"), sanitize.colnames.function=function (x) print(x))
@ 
...
lotrquestions <- read.csv("lotritems.csv")
print(xtable(lotrquestions, label="tab:lotritems", caption="Life Orientation Test, Revised Item Content"))
@ 

Table~\ref{tab:lotritems} shows the content of the items.
...
lotr1.xtab <- FactorXtab(lotr.fact.1, names=c("Optimism"),label="tab:hom1lotr1fact", caption="Factor Loadings, One Factor Solution, LOT-R, Sample One", sanitize.text.function= function (x) x)
...
All of the items meet the assumptions of a uni-dimensional scale.  Next, the item orderings were examined.
...
print(xtable(coefirt(lotr.pcm.2pl),label="tab:lotr2plestimates", caption="Coefficient Estimates for LOTR Two Paramter Graded Response Model"), sanitize.colnames.function=function (x) print(x))
@
...
print(xtable(coefirt(lotr2b.grm.1pl), caption="Coefficient Estimates for One Parameter Graded Response Model, LOT-R, Split B", label="tab:lotr2bgrm1pl"), sanitize.colnames.function=function (x) print(x))
@ 
...
The two parameter GRM (Appendix, Page~\pageref{tab:lotr2bgrm2pl}) suggested that the ability estimates have risen while the discrimination parameters have fallen for the majority of the items (except for 7). 
...
print(xtable(coefirt(lotr2c.grm.1pl), caption="Coefficient Estimates for LOT-R One Parameter Graded Response Model, Split C", label="tab:lotr2cgrm1pl"), sanitize.colnames.function=function (x) print(x))
@ 
...
When the estimates for the two parameter GRM (not shown) were examined, it can be seen that Q7 is  the most discriminating question, while still having the lowest ability estimates, suggesting that it is a very good question for separating out optimism and pessimism. Q10 is still the most difficult, but not as discriminating as Q7. 
...
cors <- scale.cor$r
print(xtable(as.data.frame(cors), label="tab:hom2scalecor",  caption="Spearman Correlations between Scale Totals across both Samples" ), environment="sidewaystable", scalebox=0.9)
...
print(xtable(apareg(health.step.test), caption="Coefficients for General Health Regression Model with predictors chosen by Stepwise Selection on unseen data, Sample One", label="tab:hom1healthsteptest"))
@ 
...
print(xtable(apareg(opt.step), caption="Coefficients for Stepwise Selected Regression Model on Optimism", label="tab:hom1stepopttest"))
@ 
...
print(xtable(apareg(opt.step.test), caption="Coefficients for Stepwise Selected Model on Test Data, General Health", label="tab:homstepopttest2a"))
@ 



As can be seen from Table \ref{tab:homstepopttest2a}, the stepwise model has an extremely significant coefficient for emotional well being and general health, and non-significant coefficients for  energy fatigue, pain and mindfulness. The coefficient signs are all in line with those from study one. The RMSEA metric gave a value of 0.746. This confirmation between the model fit on a subset of Sample One and that fit on a subset of Sample Two increases the confidence that we should have in this solution.  
...
print(xtable(apareg(opt.step), caption="Coefficients for Stepwise Selected Regression Model (using AIC(k=3)), forward and backward selection from a full model", label="tab:hom1optstep"))
@ 
...
print(xtable(apareg(mind.step.test), caption="Coefficients for Stepwise Selected Regression on Mindfulness", label="tab:hom1mindsteptest"))
@ 
