...
print(xtable(maas2cfit,label="tab:tcq2cmaassemcomp", caption=" Comparison of Fit Indices for MAAS Factor Solutions, Sample Two Split B"), scalebox=0.8)
@
...
As can be seen from Table \ref{tab:maas2bsemcompare}, the one factor solution again performs best, further increasing our confidence in its adequacy.  Next,  item response modelling is applied to this scale. 
...
The next stage in the analyses was to examine the 
