...
print(xtable(rand.scales),,label="tab:randscales")
@
...
print(xtable(coef(physfun.gpcm.rasch),label="tab:physfunrasch"))
@
...
<<physfunraschprint, echo=FALSE, results=tex>>=
print(xtable(coef(physfun.gpcm.1pl),label="tab:randphysfun1pl"))
@ 
...
print(xtable(coef(physfun.gpcm.gpcm),label="tab:randphysfungpcm"))
@ 
...
print(xtable(coef(physfun.grm.1pl), label="tab:physfungrm1", caption="Coefficients for One Parameter Graded Response Model, IRT Physical Functioning Scale"))
@ 
...
<<physfungrm1plprint, echo=FALSE, results=tex>>=
print(xtable(coef(physfun.grm.2pl), label="tab:physfungrm2", caption="Coefficients for One Parameter Graded Response Model, IRT Physical Functioning Scale"))
@ 


From Table \ref{tab:phyfungrm2}, it can be seen that the allowing the discrimination parameter to vary freely makes a large difference to the coefficient estimates. Note that Q7 appears to be the most discrimination question, with a slope of 5.64. The next process was to assess if the two parameter model provided a significant improvement in fit over and above the one parameter model.
...
print(xtable(as.data.frame(unlist(coef(emhealth.pcm.rasch))),label="tab:emhealthrasch"))
@
...
print(xtable(coef2mat(coef(emhealth.pcm.1PL))),label="tab:emhealth1pl"))
@
...
The next scale to be examined is the  General Health scale.  The first analysis undertaken was to fit a partial credit rasch model to the scale.

<<genhealthPCM, echo=FALSE, results=tex>>=
irtenergy.pcm.rasch <- gpcm(na.omit(irtenergy), constraint="rasch")
print(xtable(coef(irtenergy.pcm.rasch), label="tab:energypcmrasch", caption="Coefficients for IRT Energy Scale, Rasch Partial Credit Model"))
@
...
print(xtable(coef2mat(coef(irtenergy.pcm.1pl)), label="tab:energypcm1pl", caption="Coefficients for IRT Energy Scale, One parameter Partial Credit Model"))
@

<<genhealthpcm2pl, echo=FALSE, results=hide>>=
irtenergy.pcm.2pl <- gpcm(na.omit(irtenergy), constraint="gpcm")
print(xtable(coef2mat(coef(irtenergy.pcm.2pl)), label="tab:energypcm2pl", caption="Coefficients of IRT Energy Scale PCM, Two Parameter Model"))
@
...
print(xtable(coef(maas.grm.1pl), caption="Coefficients for MAAS One Parameter Graded Response Model", label="tab:maasgrm1pl"))
@ 
...
print(xtable(coef(maas.grm.2pl), caption="Coefficients for MAAS Two Parameter Graded Response Model", label="tab:maasgrm2pl"))
...
The estimated parameters of the model can be seen in Table \ref{tab:maasgrm2pl}.
...
print(xtable(coef(lotr.pcm.2pl),label="tab:lotr2plestimates", caption="Coefficients for LOTR Two Paramter Graded Response Model"))
@
...
maascomp.xtab2 <- xtable(maascomp2,label="tab:maassemcompare2")
...
lotrcomp.xtab2 <- xtable(lotrcomp2,label="tab:lotrcompare2")
