...
Additionally, as the new method proposed to predict the placebo response in healthy volunteers involved the use of implicit measures (two IAT) (c.f. \ref{cha:literature-review}) it was also necessary to collect data on the construct of mindfulness as operationalised using the Mindful Attention Absorption Scale (MAAS), as this construct has been shown to mediate the relationship between explicit and implicit measures. 

Over the course of the study of the placebo, there have been few individual-level psychological predictors of the effect which have been reproduced. Many studies have investigated the variables believed to be associated with this phenomenon, but most of them have not replicated in future studies. Indeed, some authors claim that the search is fruitless \cite{Shapiro1997} while others argue that the placebo effect is shaped more by the situation, and therefore there is no such thing as a placebo responder \cite{Kaptchuk2008a}. 
...
As shown in Table \ref{tab:hom1stepopttest} the model performs much more poorly on unseen data. For a further discussion of this relationship, see Chapters \ref{cha:primary-research} and \ref{cha:general-discussion}. 
...
As has been shown above, the  model showed that emotional well being and Age were important, and general health tended not to be removed from any of the models, though it did not have a particularly large coefficient. Note that because the p-values reported for the regressions were calculated based on a subset of the data not used for selection of the predictors, these can be regarded as accurate. 
...
As can be seen from Table \ref{tab:hom1mindsteptest}, the stepwise selection process retained role limitation, emotional role limitations, emotional well being and energy fatigue. 
...
@ 
As can be seen from Table \ref{tab:hom1healthsteptest}, optimism, role limitations and energy fatigue were retained as predictors in the model. None of the relationships were significant, though the coefficient for the effect of optimism was in line with the correlations and previous analyses carried out above. 
...
print(xtable(randsemcomp,label="tab:randsemcomparehom2", caption="Model Comparison for RAND MOS data using Models from Sample 1 on Sample 2" ))
@
...
As can be seen from Table \ref{tab:homstepopttest2a}, the stepwise model has an extremely significant coefficient for emotional well being, and a moderately significant coefficient for general health and age. The coefficient signs are all in line with those from study one. The RMSEA metric gave a value of 0.746. Figure \ref{fig:optstepredplot} shows this in more detail. This confirmation between the model fit on a subset of Sample One and that fit on a subset of Sample Two increases the confidence that we should have in this solution.  
...
The summary statistics for the Sample Two are shown in Table \ref{tab:hom2tots}. It can be seen that both general health and optimism levels were somewhat lower than in Sample One. This may perhaps be the result of more precise estimation due to a larger sample size, or potentially the impact of the online administration. 
...
rownames(mass2c.grm.test.all) <- c("One Parameter", "Two Parameter")
@ 
