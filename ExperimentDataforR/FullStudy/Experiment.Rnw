...
Participants in the treatment group were told that \textit{the cream was a potent painkiller, recently approved, which would take effect almost immediately}. Participants in the placebo group were told that \textit{they were receiving a placebo and that placebos have been clinically proven to reduce pain, and that it would take effect almost immediately.}
...
#setwd("Data/Experiment Data for R/FullStudy/")
expmeasures <- read.csv("explicitmeasuresfixeddrop1991.csv")
expfull <- read.csv("expmeasuresremove1991.csv"  )
vasscores <- read.csv("VASscores.csv"       )
optiat <- read.csv("optiatres.csv" )
tcqiat <- read.csv("tcqiatres.csv")
healthmodels <- new.env()
tcqmodels <- new.env()
load("~/Thesis/HealthDataforR/healthforthesis.rda", envir=healthmodels)
load("~/Thesis/tcqdata180312.rda", envir=tcqmodels)
@ 
...
optmeanconf.xtab <- xtable(optmeanconf)
...
\scalebox[0.7]{\begin{figure}
<<optbootplot, echo=FALSE, fig=TRUE>>=
print(plot(optboot))
@   
  \caption{Plot of Bootstrap Results for the Optimism IAT}
...
print(plot(tcqboot))
@   
  \caption{Bootstrap Results for Treatment Credibility IAT score}
...
pairs.panels(tcqstimblock3[,10:17])
@  
  \caption{Scatterplot Matrix for Treatment Credibility IAT, Block 3}
...
pairs.panels(tcqstimblock5[,10:17])
@   
  \caption{Scatterplot Matrix for Treatment Credibility IAT Stimuli, Block 5}
  \label{fig:optstimbklock5}
\end{figure}
...
pairs.panels(optstimblock3[,8:15])
@   
  \caption{Scatterplot Matrix for Optimism IAT Stimuli, Block 5}
...
pairs.panels(optstimblock5[,8:15])
@   
  \caption{Scatterplot Matrix for Optimism IAT Stimuli, Block 5}
  \label{fig:optstimblock5}
\end{figure}
...
pairs.panels(optiatsorted[5:10])
@   
...
opt.demo <- xtable(summary(lm(BlockTime~Block+Age+Gender, data=optiatdemographics), label="tab:optiatdemo", caption="Test of Demographics and their effec on Response Time by Block on the Optimism IAT"))
print(opt.demo)
@  

As can be seen from Table \ref{tab:optiatdemo}, the major influence on Block Time comes from Block, which is as expected given that Blocks 3 and 5 had three times as many trials as the other blocks. However, there is also an effect of gender, with men tending to respond somewhat quicker than females. This is interesting, as there are typically no gender based effects on IAT's (except for those which measure gender attitudes). 

<<survivaldata, echo=FALSE, results=hide>>=
painratings <- vasscores[,c(1,9:53)]
napainratings <- apply(painratings[,2:length(painratings)],1, function (x)  sum(is.na(x)))
lengthsurv <- 46-napainratings
survlength <- data.frame(Participant=painratings$Participant, lengthsurv=lengthsurv)
Iatandexpmeasures <- merge(Iatandexpmeasures, survlength, by="Participant")
@ 
...
painratings.ts <- apply(painratings.trans,2, as.numeric)
painratings.ts.df <- as.data.frame(painratings.ts)
@ 

<<getcountsbelow7, echo=FALSE, results=hide>>=
painratings.7 <- apply(painratings.ts.df, 2, function (x) which(x==7, arr.ind=TRUE))
getLenbelow7 <- function (x) {
  if(length(x)==0 |length(x)== 1) {
    return(0)
    }
  else {
    maxind <- x[length(x)]
    indless <- x[(length(x)-1)]
    return(maxind-indless)
    }
  }
lenbelow7 <- sapply(painratings.7, getLenbelow7)
lenbelow7.df <- as.data.frame(lenbelow7)
lenbelow7.df$Participant <- names(lenbelow7)
@ 
...
conv.xtab <- xtable(summary(aov(meanconv~Condition, data=Iatandexpmeasures)), label="tab:convcond", caption="Mean Conventional Credibility Score by Condition")
alt.xtab <- xtable(summary(aov(meanalt~Condition, data=Iatandexpmeasures)), caption="Mean Alternative Scores by Condition", label="tab:altcond")
...
<<explicitfullsubset, echo=FALSE, results=hide>>=
lotrexp <- expfull[,grep("LOTR", x=names(expfull))]
maasexp <- expfull[,grep("MAAS", x=names(expfull))]
@ 

<<irtabilityestimateshom, echo=FALSE, results=hide>>=
maas.grm.1pl <- with(healthmodels, maas.grm.1pl)
maas.grm.2pl <- with(healthmodels, maas.grm.2pl)
lotr.grm.2pl <- with(healthmodels, lotr.grm.2pl)
lotr.grm.1pl <- with(healthmodels, lotr.grm.1pl)
lotr.exp.fscores1pl <- factor.scores(lotr.grm.1pl, resp.patterns=lotrexp,na.action="na.exclude")
lotr.exp.fscores.2pl <- factor.scores(lotr.grm.1pl, resp.patterns=lotrexp,na.action="na.exclude")
maas.exp.fscores1pl <- factor.scores(maas.grm.1pl, resp.patterns=maasexp,na.action="na.exclude")
maas.exp.fscores.2pl <- factor.scores(maas.grm.2pl, resp.patterns=maasexp, na.action="na.exclude")
lotr1pl.abilities <-lotr.exp.fscores1pl$score.dat["z1"]
lotr2pl.abilities <-lotr.exp.fscores.2pl$score.dat["z1"]
maas2pl.abilities <-maas.exp.fscores.2pl$score.dat["z1"]
maas1pl.abilities <-maas.exp.fscores1pl$score.dat["z1"]
abilities <- as.data.frame(cbind(lotr1pl.abilities, lotr2pl.abilities, maas2pl.abilities, maas1pl.abilities))
names(abilities) <- c("lotr1.irt", "lotr2.irt","maas1.irt", "maas2.irt")
expmeasuresfull <- cbind(expfull, abilities)
## expmeasureswithirt <- merge(expmeasuresfull, Iatandexpmeasures, by="Participant")
## expmeasuresirtpainratings <- merge(expmeasureswithirt, lenbelow7.df, by="Participant")
@ 
\begin{figure}
...
print(lotrmaas.comp)
@   
  \caption{Scatterplot of Optimism Versus Mindfulness}
  \label{fig:lotrmaas}
\end{figure}
...
@   
  \caption{Scatterplot Matrix of Implicit and Explicit Measures}
  \label{fig:iatexp}
\end{figure}
...
print(optiat.PImap)
@   
  \caption{Optimism IAT Rasch Model Person-Item Map}
  \label{fig:optpimap}
\end{figure}
...
print(tcqiat.piplot)
@   
  \caption{Treatment Credibility IAT Rasch Model Person-Item Plot}
  \label{fig:tcqiatpiplot}
\end{figure}

Above in Table \ref{tab:tcqiatrasch} are shown the estimated abilities and their associated standard errors for the Treatment Credibility IAT. In Figure \ref{fig:tcqiatpiplot} the estimates of person and item abilities can be seen in graphical form. 
...
@   
  \caption{Boxplot of Treatment Credibility IAT scores by Gender}
  \label{fig:tcqiatgender}
\end{figure}



As can be seen from Figure \ref{fig:tcqiatgender} above, there were no significant differences between men and women in  the sample with regards to their scores on the Treatment Credibility Questionnaire. However, the variance was much higher for men, which was a pattern replicated in previous research into Treatment Credibility (using a self report instrument).
\begin{figure}
...
@   
  \caption{Boxplot of Optimism IAT Scores by Gender}
...
@   
  \caption{Mean Pain Ratings by Condition Over Time}
  \label{fig:meanpaintsplot}
\end{figure}



As can be seen from Figure \ref{fig:meanpaintsplot}, the placebo group tended to report lower mean pain ratings across time. This was an extremely unexpected finding, and therefore the pain ratings were reanalysed using medians, to downgrade the effects that outliers could be having on the results. The curves shown above used a locally weighted smoother to create the lines, given the substantial non linearity of the results. However, this plot does show that there was a significant difference between the placebo group and the two other conditions. 
...
@   
  \caption{Median Pain Ratings by Condition Over Time}
...
@   
  \caption{Mean Pain Ratings by Group Autocorrelation Plots}
  \label{fig:meanpaintscorrelationplots}
\end{figure}
...
print(surviat.comp)
@   
  \caption{Length spent in the Experiment by Condition and Treatment Credibility IAT score (using a loess smoother)}
  \label{fig:ggplotsurvivalgroupiat}
\end{figure}

\begin{figure}
<<placresponsesurvtcqiatscore, echo=FALSE, fig=TRUE>>=
...
print(surviat2.comp)
@   
  \caption{Time spent in the Experiment by Response to Placebo and Treatment Credibility IAT score}
  \label{fig:placresponsesurvtcqiatscore}
\end{figure}


As can be seen above in Figure \ref{fig:placresponsesurvtcqiatscore}, the group which responded to placebo tended to have a much higher tcqiatscore. This is in line with the hypotheses of the experiment, and leads on to formal test of the model expressed graphically above. Note that the difference between the groups in terms of placebo response does not appear to be significant (the dark area around the lines represent the standard errors of the loess fit). 
...
print(optiatcorr2)
@   
  \caption{Time spent in the Experiment by Placebo Response and Optimism IAT score}
  \label{fig:optiatcorrsurv}
\end{figure}
...
@   
  \caption{Time spent in the Experiment by Condition and Cream Credibility Score}
...
print(placrespplot2)
@   
  \caption{Pain Ratings over Time by Whether or not the Participant Responded to Placebo}
...
As can be seen above in Table \ref{tab:placmod1}, the treatment credibility IAT was a significant independent predictor of placebo response, confirming the major hypothesis of the thesis. 
...
As shown in Table \ref{tab:placmod2} the Optimism IAT is not an independent significant predictor of placebo response. 
...
placmod6<- glm(PlacResp~tcqiatscore*optiatscore*LOTR, data=Iatandexpmeasures, family=binomial(link="logit"))
...
lotriatplot <- ggplot(iatexpyesno, aes(x=LOTR, y=tcqiatscore , colour=PlacResp))
lotriatplot2 <- lotriatplot+layer(geom="smooth")
print(lotriatplot2)
@   
  \caption{Plot of Pain Ratings over Time by Placebo Response, TC IAT and Optimism Scores}
  \label{fig:3wayinter}
\end{figure}
...
print(lotrtcqplot2)
@     
   \caption{Optimism against Treatment Credibility IAT score by Placebo Response}
   \label{fig:lotrtcqsmoothplot}
 \end{figure}
...
print(lotroptplot2)
@   
  \caption{Optimism against Optimism IAT by Placebo Response}
...
\subsection{Modelling Time Spent Below Pain Rating of Seven}
\label{sec:modelling-time-spent}

The next stage in the analysis of the placebo response data was to examine the number of minutes spent underneath the breakpoint of seven. This was modelled using a zero-inflated Poisson linear model, as this is the appropriate response function for count data. 

The first step was to examine the relationship between the TCQ iat score and the amount of minutes spent under a pain rating of seven (after the pain ratings had reached seven).

<<placmodpoistcq, echo=FALSE, results=tex>>=
## placmodpois1 <- glm(lenbelow7~tcqiatscore, data=, family=quasipoisson(link="log"))
## print(xtable(summary(placmodpois1), label="tab:placmodpois1", caption="Poisson Model for Length of time spent underneath seven given TCQ iat score"))
@ 

As can be seen from Table \ref{tab:placmodpois1}, the TCQ IAT score was not a useful predictor of the amount of time spent below a pain rating of seven.

The next step was to examine the effect of Optimism IAT score on the length of time a placebo response lasted. 

<<placmodposiopt, echo=FALSE, results=tex>>=
## placmodpois2 <- glm(lenbelow7~optiatscore, data=expmeasuresirtpainratings, family=quasipoisson(link="log"))
## print(xtable(summary(placmodpois2), label="tab:placmodpois2", caption="Poisson Model for Length of time spent underneath seven given Optimsism IAT score"))
@ 

As shown in Table \ref{tab:placmodpois2}, Optimism IAT score was not a significant predictor of the length of time spent below a pain rating of seven.

Next, the effects of explicit optimism were examined. 

<<placmodposiopt, echo=FALSE, results=tex>>=
## placmodpois3 <- glm(lenbelow7~LOTR, data=expmeasuresirtpainratings, family=quasipoisson(link="log"))
## print(xtable(summary(placmodpois3), label="tab:placmodpois3", caption="Poisson Model for Length of time spent underneath seven given LOTR score"))
@ 

Table \ref{tab:placmodpois3} demonstrates the the Life Orientation Test scores also had no impact on how long a participants pain ratings stayed below seven. 

<<placmodposiopt, echo=FALSE, results=tex>>=
## placmodpois4 <- glm(lenbelow7~LOTR*tcqiatscore*optiatscore, data=expmeasuresirtpainratings, family=quasipoisson(link="log"))
## print(xtable(summary(placmodpois4), label="tab:placmodpois4", caption="Poisson Model for Length of time spent underneath seven given LOTR, TC IAT and Optimism IAT score"))
@ 

Table \ref{tab:placmodpois4} shows the effect of TC IAT, Optimism IAT and LOT-R scores and their interactions. It can be seen that, contrary to the results of the logistic regression, there are no significant effects shown in this model. 
...
names(biologicaldata) <- biodata2
write.csv(biologicaldata, file="GSRdata.csv")
pbionotna <- apply(biologicaldata, 2, function (x) sum(!is.na(x)))
gsr.full <- biologicaldata[1:31550,]
gsr.full.ts <- as.data.frame(lapply(gsr.full, ts, frequency=1000))
gsr.agg <- lapply(gsr.full.ts,  function(x) aggregate(as.ts(x), FUN=mean))
gsr.agg.df <- as.data.frame(gsr.agg)
@ 
...
<<getECG, echo=FALSE, results=hide>>=
ecgmat <- matrix(NA, nrow=35550, ncol=107)
getdata <- function(data) {
  for(i in 1:length(biodata)) {
  temp <- get(data[i])
  print(i)
  ecgmat[1:nrow(temp),i] <- temp[1:nrow(temp),2]
}
  ecgmat
}
ecgdata <- getdata(biodata)
ecgnames <- gsub("phys", "ECG", x=biodata)
ecgdata <- as.data.frame(ecgdata)
names(ecgdata) <- ecgnames
@ 
...
partsurvmin <- Iatandexpmeasures[,c("Participant","Condition","Gender", "PlacResp", "lengthsurv")]
...
@   
  \caption{Median GSR over time by Condiition}
  \label{fig:mediangsr}
\end{figure}
...
\begin{figure}
<<ggplotmeangsr, echo=FALSE, fig=TRUE>>=
...
print(meangsr2)
@   
  \caption{Mean GSR levels by Condition (using a loess smoother)}
  \label{fig:meangsrplot}
\end{figure}

\begin{figure}
<<ggplotmedgsr, echo=FALSE, fig=TRUE>>=
medgsr.melt <- melt(medgsr, id="Time")
medgsrplot <- ggplot(medgsr.melt, aes(x=Time, y=value, label=variable, colour=variable))
medgsr2 <- medgsrplot+layer(geom="smooth")
print(medgsr2)
@   
  \caption{Median GSR scores by Condition (using a loess smoother)}
  \label{fig:medgsrplot}
\end{figure}
