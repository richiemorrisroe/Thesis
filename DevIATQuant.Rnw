...
qual <- read.csv("RepGridPeople.csv", stringsAsFactors=FALSE, na.strings="", header=FALSE)
...
qual.tab <- as.data.frame(table(vals2))
qual.tab.sort <- qual.tab[with(qual.tab, order(Freq, decreasing=TRUE)),]
@ 

\begin{figure}
<<plotfreq, echo=FALSE, fig=TRUE>>=
freq.pl<-ggplot(qual.tab.sort[1:20,], aes(x=vals2, y=Freq))+geom_bar(stat="identity")+theme(axis.text.x=element_text(angle=-45))
print(freq.pl)
@   
...
The first step was examining the correlation between the two implicit measures. This was found to be 0.42, using Kendall's $\tau$. This is somewhat higher than would be hoped for, but still only represents 16\% of the variance in common between the two measures. 
