...
qual <- read.csv("RepGridPeople.csv", stringsAsFactors=FALSE, na.strings="")
qual.m <- melt(qual, id.vars=NULL)
qual.m[,"value"] <- with(qual.m, tolower(value))
qual.m2 <- qual.m[with(qual.m, gsub("s$", "", x=value)),]
qual.tab <- as.data.frame(with(qual.m2, table(value)))
qual.tab.sort <- qual.tab[with(qual.tab, order(Freq, decreasing=TRUE)),]
@ 

\begin{figure}
<<plotfreq, echo=FALSE, fig=TRUE>>=
freq.pl<-ggplot(qual.tab.sort[1:20,], aes(x=value, y=Freq))+geom_bar(stat="identity")+theme(axis.text.x=element_text(angle=-45))
print(freq.pl)
@   
...
The IAT measures were piloted on 17 students from the psychology department. Each of these students were administered either the LOT-R and the optimism IAT or the TCQ and the TCQ-IAT. 
...
source("func.R")
optiat <-read.table("ExperimentDataforR/Pilot/Opt IAT Pilot data 191110 FINISHED.csv", header=TRUE, sep="\t" )
...
The mean Optimism IAT score was 0.5833, and the distribution can be seen in Figure \ref{fig:optiatpilot}. As can be seen the majority of participants had a positive IAT score, representing their ability to respond quicker in the congruent rather than incongruent condition. 
...
tcqiat <- read.csv("ExperimentDataforR/Pilot/tcq prep data for IAT.csv")
...
As can be seen from Figure \ref{fig:tcqpilotplot}, the range is greater for the TCQ IAT, potentially because of the less familiar nature of the stimuli. Unfortunately, the explicit expectancy scores are not available for this sample, nor are the demographics, and so this cannot be reported on explicitly. However, from a report prepared at the time, the correlation was equal to 0.20, which is in line with that for the optimism IAT. 
...
iepilot<-read.csv("ExperimentDataforR/Pilot/I and E for Pilot.csv")
require(psych)
require(xtable)
@

<<gender, echo=FALSE, results=tex>>=
print(xtable(as.data.frame(with(iepilot, summary(Gender))), label="tab:gendpilot", caption="Gender of Pilot Participants"))   
@

The participants in the pilot study (N=7), consisted of five men, one woman and one participant who did not report gender. 

<<age, echo=FALSE, results=tex>>=
age.xtab <-xtable(as.data.frame(with(iepilot, table(Age))))
@
The minimum age of the participants was 21, the max was 30, the mean was 25.33 while the median was 25.5. 

<<tcqiat, echo=FALSE, results=hide>>=
tcqiat<-read.csv("ExperimentDataforR/Pilot/TCQ IAT pilot data for R2712.csv")
@
...
<<realfakeplot, echo=FALSE, fig=TRUE>>=
pairs.panels(realfakewords, labels=c("Real", "Accurate", "True", "Inaccurate", "Fake", "False"))
@
...
<<convaltplot, echo=FALSE, fig=TRUE>>=
pairs.panels(convaltwords, labels=c("Pills", "Cream", "Injections", "Surgery", "Homeopathy", "Acupuncture", "Flower Essence", "Reiki"))
@
...
pairs.panels(scales, labels=c("LOTR", "MAAS", "Pill", "Cream", "Inj", "Acu", "Hom", "Rei"))
@  
...
\begin{figure}
<<plotoptpess, echo=FALSE, fig=TRUE>>=
pairs.panels(optpesswords, labels=c("Disimproving", "Worse", "Sad", "Failing", "Happy", "Succeeding", "Better", "Improving"))
@  
...
This result is somewhat obvious, as blocks three and five are combined tasks, and would be expected to show a different pattern. However, 

%%put in this plot!!!

From Figure \ref{convaltplot} it can be seen that the stimuli for conventional treatments hardly correlate at all, but the correlations for the alternative stimuli are much higher. This is a similiar pattern to what was seen in the optimism and pessimism stimuli, so there may be some process driving this effect. There is a limit, however, to how much data can be extracted from such a small sample and determining the nature of this process (if any) may be beyond this limit. 

<<iatscores, echo=FALSE, results=tex>>=
IAT<-cbind(iepilot$DIAT.Conv, iepilot$DiatOPt)
iatcorr<-with(iepilot, cor.test(DIAT.Conv, DIATOpt, use="complete.obs", method="kendall"))
@

The first step was examining the correlation between the two explicit measures. This was found to be 0.42, using Kendall's $\tau$. This is somewhat higher than would be hoped for, but still only represents 16\% of the variance in common between the two measures. 
...
The correlations with the MAAS were 0.37 (for the Treatment Credibility IAT) and 0.08 (for the Optimism IAT). 

<<srcorr, echo=FALSE, results=tex>>=
impexp.corr.opt<-with(iepilot, cor.test(DIATOpt, LOTR, use="complete.obs", method="spearman"))
@

Surprisingly (based on the results of the pilot described above), the correlation between the optimism IAT and the LOT-R was negative, but was of the same magnitude as was seen above (-0.26). 
...
iat.and.sr<-iepilot[, c("DIAT.Conv", "DIATOpt", "MAAS", "LOTR", "ConvAltRatio")]
@ 


\begin{figure}
<<iatsrplot, echo=FALSE, results=hide>>=
pairs.panels(iat.and.sr, labels=c("IAT Conv", "IAT Opt", "MAAS", "LOTR", "ConvAltRatio"))
@  
  \caption{Pairs Plot of Realtionship Between Explicit and Implicit Measures}
  \label{fig:impexplot}
\end{figure}
...
physmeasures<-read.csv("ExperimentDataforR/Pilot/Physmeasures for Pilot.csv")
@
...
<<vastrans,echo=FALSE, figure=TRUE>>=
vastrans<-VASdata*-10
...
vas.with.plac <- read.csv("ExperimentDataforR/Pilot/vaspilot.csv", header=TRUE, sep="\t")
vas.with.plac[,"PP.No"] <- with(vas.with.plac, gsub("VAS.", "", x=PP.No))
surv <- read.csv("ExperimentDataforR/Pilot/Survdatapilot.csv")
...
<<placglmconv, echo=FALSE, results=tex>>=
...
As can be seen from Table \ref{tab:tcqiatglm}, \ref{tab:optiatglm}, \ref{tab:convglm} and \ref{tab:convglm}, the relationship between the outcome variables did not appear to be large, though they were all (with the exception of the Treatment Credibility IAT) in the correct direction. Additionally, only 3 of the seven participants responded to placebo, suggesgting that an extremely large sample size would be required to show a significant effect. This problem was worked around by using a scrambled sentence task on the majority of placebo participants, along the lines of Geers, 2005. 
...
<<bpplot, echo=FALSE, fig=TRUE>>=
plot(BPdata.ts)
@
\begin{figure}
<<gsrplot, echo=FALSE, fig=TRUE>>=
plot(GSRdata.ts)
@  
...
As can be seen from Figure \ref{fig:vasplot} the trend is downwards over time. The recording device tracks between 0 and -1 so as the line goes lower, this represents increasing pain. There is definitely a downwards trend in the pain measurements, as would be expected given the design of the experiment, where pain scores increase over time. There is substantial variability in the responses, as can be seen from the wildly different lengths of the series. Two participants in the no treatment condition went for longer than 40 minutes, while none of the participants in the treatment condition did so. This is obviously a problem, given the goal of the experiment, and the results are illustrated graphically below.

\section{Discussion}
\label{sec:discussion}

A number of issues have become clear in the piloting of the IAT measures and experimental setup. The first is that the effect sizes achieved from placebo suggestions were quite small. The solution to this in the larger study was to include a priming manipulation (the scrambled sentence task) for the majority of participants, a small group not being primed to ensure that the effect of this manipulation could be tested. 
