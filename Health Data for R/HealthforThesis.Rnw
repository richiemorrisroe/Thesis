...
irtemhealth <- randitems2[,paste(rand, c(16:20, 23:32), sep="")]
irtphyslim <- randitems2[,paste(rand, c(13,15,21:22), sep="")]
irtgenhealth <- randitems2[,paste(rand, c(1, 33, 34), sep="")]
@
...
Quite a number of items failed the item ordering assumption (RAND17, RAND18, RAND19, RAND24, RAND26) and so were removed from the scale before further analysis. 
...
There was one violation of the IIO assumption (RANDQ1), and no violations of the monotonicity assumption for the General Health scale. 
...
irtGenhealth2 <- randitems.scored[,paste(rand, c(1, 33, 36), sep="")]
...
physfun.gpcm.rasch <- gpcm(na.omit(irtphysfun), constraint="rasch")
physfun.gpcm.1pl <- gpcm(na.omit(irtphysfun), constraint="1PL")
physfun.gpcm.gpcm <- gpcm(na.omit(irtphysfun), constraint="gpcm")
@
...
% physfun.rsm <- RSM(na.omit(irtphysfun.scaled))
% rsm.xtab <- IrtXtab(physfun.rsm,label="tab:tcq1physfunrsm")
% print(rsm.xtab)
% @
