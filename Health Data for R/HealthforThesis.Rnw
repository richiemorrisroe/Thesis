...
% print(FactorXtab(maas.fact.4,label="tab:tcq1tcq1maas4fact"))
% @
...
% print(xtable(maas.fact.4$r.scores,label="tab:tcq1maas4corr"))
% @
...
% print(FactorXtab(maas.fact.3,label="tab:tcq1mass3fact"))
% @
...
% print(xtable(maas.fact.3$r.scores,label="tab:tcq1maas3corr"))
% @
...
% print(FactorXtab(maas.fact.2,label="tab:tcq1maas2fact"))
% @
...
% print(xtable(maas.fact.2$r.scores,label="tab:tcq1maas2corr"))
% @
...
irtenergy2 <- randitems2[,paste(rand, c(1,20,22,26,30,34), sep="")]
@
...
print(xtable(irtgenhealth.item.ord$violations,label="tab:tcq1genhealthitemord"))
@
...
print(xtable(as.data.frame(lotr.pcm.2pl[[1]]),label="tab:tcq1lotr2plestimates"))
@
...
Rand2model <- mxModel(name="RAND2Samp1",
                      ...
                      mxPath(from=manifests, arrows=2),
                      mxPath(from=latents, arrows=2, free=FALSE, values=0.5),
                      mxData(observed=cov(na.omit(randitems2)), type="cov", numObs=364)
                      )
...
print(xtable(randsemcomp,label="tab:tcq1with2randsemcomp"))
@
...
maascomp.xtab2 <- xtable(maascomp2,label="tab:tcq1with2maassem")
...
<<randirtscales, echo=FALSE, results=hide>>=
irtphysfun <- randitems2[,paste(rand, c(3:12), sep="")]
irtemhealth <- randitems2[,paste(rand, c(1,16:20, 23:32), sep="")]
irtphyslim <- randitems2[,paste(rand, c(13,15,21:22), sep="")]
irtgenhealth <- randitems2[,paste(rand, c( 33:35), sep="")]
@
...
Quite a number of items failed the item ordering assumption (RAND1, RAND17, RAND18, RAND19, RAND24, RAND26) and so were removed from the scale before further analysis. 

<<newIrtEmHealth, echo=FALSE, results=tex>>=
irtemhealth2 <- randitems2[,paste(rand, c(16, 20,25, 27:32), sep="")]
emhealth.item.ord2 <- check.iio(na.omit(irtemhealth2))
emhealth.mono.2 <- check.monotonicity(na.omit(irtemhealth2))
print(xtable(emhealth.item.ord2$violations, caption="IIO for reduced emotional health scale", label="tab:randemhealth2iio"))
@
...
irtgenhealth <- randitems2[,paste(rand, c(33,34), sep="")]
@
