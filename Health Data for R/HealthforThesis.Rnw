...
\emph{Sample 1:} (N=\Sexpr{nrow(hom1)}) This sample was collected from social and food areas
...
return them, so the final sample was \Sexpr{nrow(na.omit(hom1))}.

\emph{Sample 2:} (N=\Sexpr{nrow(hom2)}) This
sample was collected using a website and an email distributed to all
registered students (N=15000) from 12-24th December 2009. \Sexpr{nrow(hom2)} responded, but  391 participants
did not provide any data (apart from demographics) and so were removed
from the dataset before analysis.

The total sample after this data was removed was N=\Sexpr{nrow(na.omit(hom))}.
...
print(optplot1)
@  
  \caption{Plot of General Health against optimism with a linear regression smooth line. Dark areas on plot represent the errors of estimation}
  \label{fig:optplot1}
\end{figure}
...
optplotgend <- ggplot(hom1, aes(x=generalhealth, y=optimism, colour=Gender))+layer(geom="smooth", method="lm")
print(optplotgend)
@  
...
optplotcoll <- ggplot(na.omit(hom1), aes(x=generalhealth, y=optimism, colour=College))+layer(geom="smooth", method="lm")
print(optplotcoll)
@  
...
optplot.maas<- ggplot(na.omit(hom1), aes(x=generalhealth, y=optimism, size=mindfulness))+layer(geom="point", method="lm")
print(optplot.maas)
@  
...
print(lotrage)
@  
  \caption{Regression Line of Optimism against again (linear regression smooth). Dark areas on plot surrounding line represent errors of estimation}
...
@  
  \caption{General Health against Mindfulness, Stratified by Method of Collection, linear regression smooth}
...
The results of the maximal model are shown in Table \ref{tab:final.mind} below.
...
maas.first.xtab <- xtable(maas.first.sum, caption="Maximal Regression Model for Mindfulness")
print(maas.first.xtab)
@



The model was significant, F(\Sexpr{round(maas.first.sum$fstatistic[1], 3)},round(maas.first.sum$fstatistic[2], 3)}) = \Sexpr{round(maas.first.sum$fstatistic[3], 3)} $, and the adjusted $R^2$ for the model was equal to \Sexpr{round(maas.first.sum$adj.r.squared, 3)}.  $
...
The model was significant, F(7, 557) = \Sexpr{maas.fin.sum$fstatistic} $, while the adjusted $R^2$ was equal to \Sexpr{round( maas.fin.sum$adj.r.squared, 3)} $. The residuals of the model were plotted, and they revealed no significant failures of the assumptions. 
...
The model (shown in Table \ref{tab:healthregfirst}) was significant,F(\Sexpr{round(health.sum1$fstatistic[1], 3)},round(health.sum1$fstatistic[2], 3)}) = \Sexpr{round(health.sum1$fstatistic[3], 3)} $, and the adjusted $R^2$ for the model was equal to \Sexpr{round(health.sum1$adj.r.squared, 3)}.  $ 
...
healthfin.xtab<-xtable(health.sumfin, label="tab:health.final", caption="Final Regression Model for General Health")
...
The model was significant,F(\Sexpr{round(health.sumfin$fstatistic[1], 3)},round(health.sumfin$fstatistic[2], 3)}) = \Sexpr{round(health.sumfin$fstatistic[3], 3)} $, and the adjusted $R^2$ for the model was equal to \Sexpr{round(health.sumfin$adj.r.squared, 3)}.  $ 
...
<<retainmaas, echo=FALSE,results=tex >>=
...
Looking at the eigenvalues, it is clear that one factor explains the majority of the variance. However, both the kaiser criterion and parallel analysis suggest that three factors should be retained, while the VSS and the Minimum Average Partial criterion procedures argued for one factor solutions.
...
print(FactorXtab(rand.fact.2, label="tab:rand2fact"))
@
...
print(xtable(rand.fact.2$r.scores, label="tab:tcq1rand4corr"))
@
...
print(FactorXtab(rand.fact.4, label="tab:rand4fact"))
@
...
print(xtable(rand.fact.4$r.scores, label="tab:tcq1rand4corr"))
@
...
print(FactorXtab(rand.fact.8, label="tab:tcq1rand8fact"))
@
...
This factor structure definitely makes sense, and the fit indices are relatively acceptable, although the RMSEA is a little higher than would be wanted. In addition, the items map quite well to the subscales, which further reinforces our confidenc in this solution.


<<rand8corr, echo=FALSE, results=tex>>=
print(xtable(rand.fact.8$r.scores, label="tab:tcq1rand8corr"))
@

The factor correlations are moderate (shown in Table \ref{tab:tcq1rand8corr}) and in line with expectations. The correlations are definitely too high to use an othrogonal rotation. 

<<rand8plot, echo=FALSE, fig=TRUE>>=
plot(rand.fact.8)
@
...
print(FactorXtab(rand.fact.13,label="tab:tcq1rand13fact"))
@
...
PA9: "RANDQ33". This one item relates to sickness, and can probably be termed as sickness.
...
The last two factors have only one item loading on them, and three is normally regarded as the minimum for a factor to replicate %cite Tabachnick & Fidell here.
...
print(xtable(rand.fact.13$r.scores,label="tab:tcq1rand13corr"))
@
As Table \ref{tab:tcq1rand13corr} shows, the factors are inter-correlated, but not in any coherent fashion. 
...
print(xtable(randsemcomp,label="tab:tcq1randsemcomp"))
@
...
print(FactorXtab(maas.fact.1,label="tab:tcq1maas1fact"))
@
...
\subsubsection{MAAS Seven Factor Solution}
...
print(FactorXtab(maas.fact.7,label="tab:tcq1maas7fact"))
@
...
PA3:"MAASQ13". This factor is unlikely to be real, as it consists only of one item. The item relates to preoccupations, and so is given that name. 
...
maascomp.xtab <- xtable(maascomp,label="tab:tcq1maassemcomp")
print(maascomp.xtab)
...
print(FactorXtab(lotr.fact.1,label="tab:tcq1lotr1fact"))
@
...
PA3: "LOTRQ7" "LOTRQ9". Again, this factor can best be termed Pessimism2. 


<<lotr3plot, echo=FALSE, fig=TRUE>>=
plot(lotr.fact.3)
@
...
lotrcomp.xtab <- xtable(lotrcomp,label="tab:tcq1lotrcomp")
print(lotrcomp.xtab)
@
...
print(xtable(rand.scales),,label="tab:tcq1randscales")
@
...
print(xtable(irtphys.item.ord$violations,label="tab:tcq1physfuncitemord"))
@
...
print(xtable(irtemhealth.item.ord$violations,label="tab:tcq1randemhealthitemord"))
@

Quite a number of items failed the item ordering assumption (RAND20, RAND32) and so were removed from the scale before further analysis. Interestingly enough, these items represent the Social Functioning scale. 
...
There were two violations of the item ordering assumption, RAND21 and RAND23, and these items were removed before further analyses.

<<randGenhealthItemord, echo=FALSE, results=tex>>=
print(xtable(irtenergy.item.ord$violations,label="tab:tcq1energyitemord"))
@
...
print(xtable(coef(physfun.gpcm.rasch),label="tab:tcq1physfunrasch"))
@

As can be seen from Table \ref{tab:physfunraschprint}, the Rasch model was not a good fit to these items, as RANDQ7 and RANDQ12 showed non increasing item difficulty estimates. 

<<physfunraschprint, echo=FALSE, results=tex>>=
print(xtable(coef(physfun.gpcm.1pl),label="tab:tcq1physfun1pl"))
@ 

As shown in Table \ref{tab:tcq1physfun1pl}, the one parameter model also proved a poor fit to the data, as RANDQ7 and RANDQ12 still show problems with the ordering of abilities. 

<<physfungpcmprint, echo=FALSE, results=tex>>=
print(xtable(coef(physfun.gpcm.gpcm),label="tab:tcq1physfungpcm"))
@ 

Table \ref{tab:tcq1physfungpcm} shows that RANDQ12 is still a problematic item for the two parameter model, though RANDQ7 does not show the problems seen earlier. The next step in the modelling process was to fit a one and two parameter GRM. 
...
physfun.grm.1pl <- grm(na.exclude(irtphysfun), constrained=TRUE)
physfun.grm.2pl <- grm(na.exclude(irtphysfun), constrained=FALSE)
@ 
...
print(xtable(as.data.frame(unlist(coef(emhealth.pcm.rasch))),label="tab:tcq1emhealthrasch"))
@
...
print(xtable(as.data.frame(unlist(coef(emhealth.pcm.1PL))),label="tab:tcq1emhealth1pl"))
@
...
print(xtable(as.data.frame(unlist(coef(emhealth.pcm.gpcm))),label="tab:tcq1emhealth2pl"))
@
...
print(xtable(as.matrix(maas.scales)),label="tab:tcq1maasassumptioncheck")
@
...
print(xtable(maas.iio$violations,label="tab:tcq1maasIO"))
@
...
print(xtable(summary(maas.mono),label="tab:tcq1maasmono"))
@

As can be seen from TAble \ref{tab:maasmonotonicity}, there were no violations of the monotonicity assumption for the reduced scale. The item coefficients (ItemH) are quite low, many of them hang around 0.30, which is the minimum allowed. 
...
print(xtable(coef(maas.pcm.rasch), label="tab:tcq1maaspcmrasch", caption="MAAS Partial Credit Model Coefficients, Rasch"))
@ 

As can be seen from Table \ref{tab:tcq1maaspcmrasch}, the Rasch partial credit model is not a good fit to the data. There are numerous violations of the increasing ability scores assumption. Next, a one parameter model (with discrimination estimated from the data) was fitted to the maas items. 

<<maasirtraschprint, echo=FALSE, results=tex>>=
print(xtable(coef(maas.pcm.1pl), label="tab:tcq1maaspcm1pl", caption="MAAS Partial Credit Model Coefficients, One Parameter Model"))
@ 

The estimated one parameter model is shown in Table \ref{tab:tcq1maaspcm1pl}, and it can clearly be seen that it suffers from the same problems as the rasch model fitted above. Finally, a two parameter partial credit model was fitted to these items. 

<<maasirtraschprint, echo=FALSE, results=tex>>=
print(xtable(coef(maas.pcm.2pl), label="tab:tcq1maaspcmgpcm", caption="MAAS Partial Credit Model Coefficients, Two Parameter Model"))
@ 

The Table (\ref{tab:tcq1maaspcmgpcm}) clearly shows that even the two parameter model does not provide a good fit to the data. The next stage of analysis for the Mindfulness scale was to fit one and two parameter Graded Response Models to the data. 
...
print(xtable(as.data.frame(maas.pcm.2pl[[1]]),label="tab:tcq1maas2pl"))
@

The estimated parameters of the model can be seen in Table \ref{tab:maas2plprint}.

The three models were subjected to anova comparison and the 1 parameter model was significantly (p<0.001) better than the rasch model, and the 2 parameter model was significantly better than the 1 parameter model (p<0.001). However, the ultimate test is the ability of the model to predict out-of-sample data. 
...
print(xtable(as.matrix(lotr.scales),label="tab:tcq1lotrscales"))
@
...
print(xtable(lotr.iio$violations,label="tab:tcq1lotrIO"))
@
...
print(xtable(summary(lotr.mono),label="tab:tcq1lotrmono"))
@
...
print(xtable(coef(lotr.pcm.rasch), label="tab:tcq1lotrpcmrasch", caption="Rasch Partial Credit Model for Life Orientation Test, Revised"))
@ 

As shown in Table \ref{tab:tcq1lotrpcmrasch}, the model does not provide a good fit to the data. There are numerous violations of the ordering assumptions of the model. Next, a one parameter model was fit to the data. 

<<lotrpcm1plprint, echo=FALSE, results=tex>>=
print(xtable(coef(lotr.pcm.1pl), label="tab:tcq1lotrpcm1pl", caption="One parameter Partial Credit Model for Life Orientation Test, Revised"))
@ 

As can be seen in Table \ref{tab:tcq1lotrpcm1pl}, there are again some violations of the ability ordering assumption (LOTR4, category 3). Next, a two parameter PCM was fitted to the LOTR items. 

<<lotrpcm1plprint, echo=FALSE, results=tex>>=
print(xtable(coef(lotr.pcm.2pl), label="tab:tcq1lotrpcmgpcm", caption="Two Parameter Partial Credit Model for Life Orientation Test, Revised"))
@ 

The coefficients in Table \ref{tab:tcq1lotrpcmgpcm} show that again, LOTR4 ensures that the model does not fit correctly. 
...
print(xtable(coef(lotr.pcm.2pl),label="tab:tcq1lotr2plestimates"))
@
...
rand6pred <- predict.psych(rand.fact.2, randitems2)
rand5pred <- predict.psych(rand.fact.4, randitems2)
rand4pred <- predict.psych(rand.fact.8, randitems2)
rand13pred <- predict.psych(rand.fact.13, randitems2)
...
print(xtable(randsemcomp,label="tab:hom1with2randsemcomp", caption="Model Comparison for MOS data using Models from Sample 1 on Sample 2" ))
@
...
maascomp.xtab2 <- xtable(maascomp2,label="tab:hom1with2maassem")
...
lotrcomp.xtab2 <- xtable(lotrcomp2,label="tab:tcq1with2lotrcomp")
...
<<fscoreslotr1to2, echo=FALSE, results=hide>>=
lotr.pcm.rasch.scores.pred <- factor.scores(lotr.pcm.rasch, resp.patterns=lotritems2a[,2:length(lotritems2a)])
lotr.rasch.scores <- getIrtPreds(lotr.pcm.rasch.scores.pred)
lotr.pcm.1pl.scores.pred <- factor.scores(lotr.pcm.1pl, resp.patterns=na.omit(lotritems2a[2:length(lotritems2a)]))
lotr.1pl.scores.pred <- getIrtPreds(lotr.pcm.1pl.scores.pred)
lotr.pcm.2pl.scores.pred <- factor.scores(lotr.pcm.2pl, resp.patterns=na.omit(lotritems2a[2:length(lotritems2a)]))
lotr.2pl.scores.pred <- getIrtPreds(lotr.pcm.2pl.scores.pred)
@

<<fscoreslotr1to2, echo=FALSE, results=hide>>=
lotr2a.pcm.rasch <- gpcm(lotritems2a, constraint="rasch")
lotr2a.pcm.1pl <- gpcm(lotritems2a, constraint="1PL")
lotr2a.pcm.gpcm<- gpcm(lotritems2a, constraint="gpcm")
lotr2a.pcm.rasch.scores <- factor.scores(lotr2a.pcm.rasch, resp.patterns=lotritems2a)
lotr.rasch.scores2a <- getIrtPreds(lotr2a.pcm.rasch.scores)
lotr2a.pcm.1pl.scores <- factor.scores(lotr2a.pcm.1pl) 
lotr.1pl.scores2a <- getIrtPreds(lotr2a.pcm.1pl.scores)
lotr2a.pcm.gpcm.scores <- factor.scores(lotr2a.pcm.gpcm)
lotr.gpcm.scores2a <- getIrtPreds(lotr2a.pcm.gpcm.scores)
@ 
...
print(FactorXtab(maas2b.fact1,label="tab:tcq2bmaas1fact"))
@
...
print(FactorXtab(maas2b.fact2,label="tab:tcq2bmaasfact2"))
@
...
print(xtable(maas2b.semcomp,label="tab:tcq2bmaas2"))
@
...
<<randcheckassumptions, echo=FALSE, results=hide>>=
rand.scales <- aisp(na.omit(randitems2))
@

<<randscales, echo=FALSE, results=tex>>=
print(xtable(rand.scales),,label="tab:tcq1randscales")
@
As can be seen from Table \ref{tab:randscales}, the item selection procedure suggests that there are four scales in the RAND MOS.  In addition, two items do not load on any scale.  The first of these is RANDQ2, which is not surprising given that it is not supposed to score on any scale, and the second is RANDQ36 which is part of the General Health scale.  The analysis of the RAND scales continues now, but using the four scales suggested by the item selection procedure.

The scales can be termed as follows:

Scale 1: RANDQ3-RANDQ12: This maps exactly to the physical functioning scale.

Scale 2: RAND1, RANDQ16-RANDQ20,23-32. This scale incorporates the Role Limitations, Social Functioning, Emotional Well Being and Energy scales, and can best be termed as Mental/Emotional Health.

Scale 3: RAND13-RAND15,RAND21-RAND22. This maps to the role limitations and pain scale, and can probably best be termed as physical limitations.

Scale 4:  RANDQ33, RANDQ34, RANDQ35. This maps almost exactly to the General Health scale, and so retains that name.

<<randirtscales, echo=FALSE, results=hide>>=
irtphysfun2 <- randitems2[,paste(rand, c(3:12), sep="")]
irtemhealth2 <- randitems2[,paste(rand, c(1,16:20, 23:32), sep="")]
irtphyslim2 <- randitems2[,paste(rand, c(13,15,21:22), sep="")]
irtgenhealth2 <- randitems2[,paste(rand, c( 33:35), sep="")]
@

<<randphysfun, echo=FALSE, results=hide>>=
irtphys2.item.ord <- check.iio(na.omit(irtphysfun2))
irtphys2.monotonicity <- check.monotonicity(na.omit(irtphysfun2))
@
...
<<randphysfuncitemord, echo=FALSE, results=tex>>=
print(xtable(irtphys2.item.ord$violations,label="tab:tcq2physfuncitemord"))
@

As can be seen from Table \ref{tab:tcq2physfuncitemord}, there were no violations of item ordering for this sample.  This demonstrates that there are no violations of monotonicity for this subscale, and suggests that item response theory modelling can proceed, for this scale at least.

<<randemhealth, echo=FALSE, results=hide>>=
irtemhealth2.item.ord <- check.iio(na.omit(irtemhealth2))
irtemhealth2.monotonicity <- check.monotonicity(na.omit(irtemhealth2))
@

<<randEmhealthItemord, echo=FALSE, results=tex>>=
print(xtable(irtemhealth2.item.ord$violations,label="tab:tcq2randemhealthitemord"))
@

As can be seen from Table \ref{tcq2randemhealthitemord},quite a number of items failed the item ordering assumption (RAND1, RAND17, RAND18, RAND19, RAND24, RAND26) and so were removed from the scale before further analysis. 

<<newIrtEmHealth, echo=FALSE, results=tex>>=
irtemhealth2.2 <- randitems2[,paste(rand, c(16, 20,25, 27:32), sep="")]
emhealth.item.ord2 <- check.iio(na.omit(irtemhealth2))
emhealth.mono.2 <- check.monotonicity(na.omit(irtemhealth2))
print(xtable(emhealth.item.ord2$violations, caption="IIO for reduced emotional health scale", label="tab:tcq2randemhealth2iio"))
@

As shown in Table \ref{tab:tcq2randemhealth2iio}, the removal of items from the emotional health scale appears to have removed the problems with invariant item ordering. The reduced scale had no issues with non-monotonic item responses. 



<<randpain, echo=FALSE, results=hide>>=
irtphyslim2.item.ord <- check.iio(na.omit(irtphyslim2))
irtphyslim2.monotonicity <- check.monotonicity(na.omit(irtphyslim2))
@
...
<<randgenhealth, echo=FALSE, results=hide>>=
irtgenhealth2.item.ord <- check.iio(na.omit(irtgenhealth2))
irtgenhealth2.monotonicity <- check.monotonicity(na.omit(irtgenhealth2))
irtgenhealth2 <- randitems2[,paste(rand, c(33,34), sep="")]
@

There was one violation of the IIO assumption (RANDQ35), and no violations of the monotonicity assumption for the General Health scale. 

<<randGenhealthItemord, echo=FALSE, results=tex>>=
print(xtable(irtgenhealth2.item.ord$violations,label="tab:tcq2genhealthitemord"))
@
...
<<physfunrasch, echo=FALSE, results=hide>>=
physfun.gpcm.rasch <- gpcm(na.exclude(irtphysfun2), constraint="rasch")
physfun.gpcm.1pl <- gpcm(na.exclude(irtphysfun2), constraint="1PL")
physfun.gpcm.gpcm <- gpcm(na.exclude(irtphysfun), constraint="gpcm")
@
...
<<maasgrmfit, echo=FALSE, results=hide>>=
maas.grm.1pl <- grm(maasitems2, constrained=TRUE)
maas.grm.2pl <- grm(maasitems2, constrained=FALSE)
@ 


<<lotrgrmfit, echo=FALSE, results=hide>>=
lotr.grm.1pl <- grm(lotritems2, constrained=TRUE)
lotr.grm.2pl <- grm(lotritems2, constrained=FALSE)
@ 
