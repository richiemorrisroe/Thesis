...
This finding should be tempered with other research that indicated that there was no correlation between the amount of dopamine released and the size of the placebo response~\cite{Scott2007a}. However, correlations only test for linear relations, and many relationships in the body are non-linear, suggesting that this finding may not be particularly robust. Also the Scott \textit{et al} study used quite a small sample, and so these results would benefit from replication with larger samples \cite{vul2009puzzlingly}.
