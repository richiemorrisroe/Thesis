...
print(xtable(tcq2.fact6.av, label="tab:tcq2fact6average",caption= "Average of Six Factor Solutions over Splits B, C and D"), sanitize.text.function = function (x) x)
@ 
...
Next, the assumptions were checked for the alternative items, and there were no violations for either the IIO or monotonicity assumptions. 
