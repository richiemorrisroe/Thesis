...
stargazer(tcq2.demo1, tcq2.demo3, summary=TRUE, title="TCQ2 Summary Statistics", label="tab:tcq2sumstats")
@ 



Many respondents stopped answering questions throughout the demographic variables. The reasons for this are unknown, but are probably not related to the design in any substantial way. The median age is 27, which shows that most of the sample was made up of students.
...
The average of the two five factor solutions (shown in Table \ref{tab:tcq2bfact5av} merged the Pill credibility and Injection credibility items together, and this pattern was repeated across both splits. Note that both the Cream and Homeopathy factors have slightly lower loadings than do the others, and the Pill \& Injections factor is weighted more towards Injections.  
...
In all cases, the seven factor model showed better fit to the data than did either the five or six factor model. There was typically not a large amount of difference between the six and seven factor models, and the six factor structure would be certainly preferred in terms of clarity and interpretation.
...
print(xtable(tcq2.grm.1pl.av, label="tab:tcq2grm1plaverage", caption="Item Difficulty Thresholds averaged over Splits B, C and D"))
@ 
...
print(xtable(as.data.frame(tcq2.conv.averror), label="tab:tcq2grmaverrorconv", caption="Average Error of Approximation for One and Two Parameter GRM's Across Splits B, C and D"))
@ 
...
print(xtable(as.data.frame(tcq2.alt.averror), label="tab:tcq2grmaverroralt", caption="Average Error of Approximation for One and Two Parameter GRM's Across Splits B, C and D"))
@ 
