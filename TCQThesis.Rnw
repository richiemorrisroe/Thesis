...
print(arrangeGrob(tcq1.1pl.pl+theme(legend.position="none"),tcq1.2pl.pl))
@   
...
<<tcqaltgrminfo, echo=FALSE, results=hide>>=
@ 
\begin{figure}
<<tcqalticcpl, echo=FALSE, fig=TRUE>>=
par(mfrow=c(1, 2))
plot(tcqalt.grm.constrained, type="IIC")
plot(tcqalt.grm2, type="IIC")  
@ 
  \caption{Item Information Curves, TCQ Alternative Scale, One Parameter GRM (left) and Two Parameter GRM (right)}
...
\begin{figure}
<<iictcqconv2grmpl, echo=FALSE, fig=TRUE>>=
par(mfrow=c(1, 2))
plot(tcq2d.conv.grm.1pl, type="IIC")
plot(tcq2d.conv.grm.2pl, type="IIC")
@   
  \caption{Item Information Curves, TCQ Conventional, Split 2, One Parameter GRM (left) and Two Parameter GRM (right)}
...
The results for the held out data are shown in Table \ref{tab:tcq2creamholdout}. The model was again significant, with an $F(6,144)=20.61$, and a p-value of $p\le 0.001$. The adjusted $R^2$ for the model was equal to 0.4497. Pill credibility, experience  cream treatments and Acupuncture credibility remained significant, along with Experience with Cream treatments, and Pill credibility, suggesting that individuals with more faith in Pill painkilling treatments were likely to endorse Cream treatments also.  
