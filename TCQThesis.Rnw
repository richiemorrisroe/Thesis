...
In this sense, what was measured were the self-reported treatment expectancies of the sample(s). These expectancies are relevant both because they may relate to the placebo response to treatments, and additionally because they may shape the choice of treatment~\cite{Bausell2005,Benedetti2005} (c.f. Section~\ref{sec:beli-about-treatm}).  
...
Additionally, in another study investigating the impact of acupuncture analgesia on recovering from dental pain following surgery, a similar effect was observed~\cite{Bausell2005}. This suggests that beliefs about treatment assignment are as important as is treatment assignment itself~\cite{Benedetti2005}. The relevance to this work is that these studies have shown that belief in receiving a real treatment is associated with greater improvement. Additionally, studies on the placebo effect  have demonstrated that belief in the veracity of a substance can have profound treatment effects. Therefore, by measuring treatment related expectancies more precisely it may become possible to predict which (if any) of a set of treatments a patient will respond to best. 

\subsection{Links to theory}
\label{sec:links-theory}

There are a number of theoretical perspectives around how expectancies relate to placebo effects. The most prevalent in the field is the response expectancy perspective of Kirsch \cite{Kirsch1985,Kirsch1997} which argues that expectancies are directly responsible for observed placebo responses, and that all other variables are mediated by them. Kirsch's theory draws from some of Albert Bandura's work on self-efficacy \cite{Bandura1977}, but is quite distinct in that response expectancies are regarded as a separate type.
...
In general, treatments for pain are available in multiple different forms. Pills are commonly available as an over-the-counter supplement, creams are available for muscle pain, and injections of morphine or other painkillers are provided in a health-care setting. In one study, the most common reason for talking some form of medication was headache (9\%, N=2590 in a US sample) suggesting that these forms of treatment are routinely used~\cite{kaufman2002recent}. Another recent randomised survey of California residents determined that the point prevalence of some form of chronic pain condition in the general population was 49\% (47.0\%-51.0\%, N=3243), which would seem to suggest that the majority of the population suffers at least some pain once per month~\cite{ohayon2010chronic}. These findings would seem to suggest that pain-related treatment expectancies are important to an understanding of pain treatment and the experience of pain more generally.
This is another benefit which a useful measure for these extremely common treatment-related expectancies would provide. 
...
For these reasons, both conventional and alternative treatment stimuli were used in this study. Additionally, it was thought that there might be some useful information conveyed by the relative ordering of conventional and alternative treatment expectancies, both within and between participants. 

\subsection{Aims and Objectives}
\label{sec:aims-objectives-tcq}
...
\item The validity of the original factor structure was assessed (Study 1) using both factor-analytic (FA) and item response theory (IRT) methods 
...
\item The convergent validity of the new measure was assessed using the BMQ (Study 2);
...
This portion of the research was designed in the following fashion. In the first study (N=299), the measure was tested for reliability and to ensure that the factor structure from the previous research replicated in this new setting, and with some changes made. 

The second study (N=1329) was conducted to validate the factor structure from the first study, and to assess convergent validity through the use of the BMQ.  This study included two more forms of alternative treatment (Homeopathy and Reiki) to assess if a common factor structure could also be found for the alternative treatments. Additionally, the second sample collected demographic statistics around income, health and familiarity with all of the painkilling treatments used in the major instrument, to assess if these would have the impacts observed in previous research. 
...
In light of evidence that the IAT correlates better with explicit measures when they are relative measures (i.e. a feeling thermometer) rather than absolute measures, a non-relative measure was adapted for this research, to maximise the incremental variance explained by each measure. 
...
Data for Study Two was split into four parts of approximately 300 observations each, in line with the procedure described in Chapter~\ref{cha:methodology}. 
...
Following this, a rank-order correlation matrix for the data was calculated and analysed.

Simple reliability analyses were carried out on the scales themselves. Following this, parallel analysis, the MAP criterion and the scree plot were used to estimate the number of factors which could be extracted from the data. Further analyses were carried out in line with the methods described in Chapter \ref{cha:methodology}, Sections \ref{sec:regress-models} and \ref{sec:psych-analys-meth}, in line with the aims and objectives laid out in Section \ref{sec:aims-objectives-tcq}. In brief, the first study used all of the data available to fit psychometric models and tested these on a subset of Study Two data. Ten-fold cross-validation was used for all regression models, and all p-values reported are from the test set. 



\section{Study One - Results}

As described above, the aim of this Study was to ensure that the altered measure possessed face validity and was reliable and that the sub-scales correlated with each other in the expected fashion. 
...
print(xtab.totals, include.rownames=FALSE)
@ 
...
Next, the differences in credibility totals between Genders was examined. 
...
There were no significant differences in median Pill scores that can be attributed to the gender of the respondents. The variability was larger within the male responses, a pattern which was repeated for Cream and Injection credibility totals also. 

However, the acupuncture credibility scores showed a different pattern. The mean credibility totals were very different for females and males.  A Kruskall Wallis examined whether this visual difference was backed up with a formal analysis. The results tended towards significance (p=0.09).
...
There were differences in the credibility scores of various treatments across the colleges of study. These differences were only significant for the Cream ($p=0.0158$) and Injection ($p=0.0168$) credibility scores. 
...
print(xtable(rel.tcq[["alpha.drop"]], caption="Item reliability Statistics for the TCQ 1. Overall alpha of the scale was 0.92, following reverse scoring of the Acupuncture Items. Reverse scored items are indicated by the minus symbol. Raw/standard alpha (Cronbach's alpha),G6: Guttmans Lambda6 (squared multiple correlation) S/N: Signal/Noise ratio, alpha\\_se: Standard error of alpha estimation", label="tab:tcq1rellong"))
@ 

Table~\ref{tab:tcq1rellong} shows the $\alpha$ coefficient for each of the items, along with the Guutman Lambda 6 statistic, which measures the amount of variation in an item which can be accounted for by a linear regression of all the other items. Standardised alpha is based on the correlations rather than the covariances, and as such as typically a much more useful measure. 
...
pillnames <- paste(c("Pill"), 1:6, sep="")
creamnames <- paste(c("Cream"), 1:6, sep="")
injnames <- paste(c("Inj"), 1:6, sep="")
acunames <- paste(c("Acu"), 1:6, sep="")
itemnames1 <- c(pillnames, creamnames, injnames, acunames)
tcq.fact.4<-fa(na.omit(tcqall), 4, rotate='oblimin', fm="pa")
tcq4.fact.xtab <- FactorXtab(tcq.fact.4,names=c("Acupuncture", "Cream", "Inj", "Pills"), label="tab:tcq1fact4", caption="Four Factor Solution, TCQ 1 Oblimin rotation")

rownames(tcq4.fact.xtab) <- itemnames1
print(tcq4.fact.xtab, sanitize.text.function= function (x) x)
@ 
...
tcq4.cor.xtab <- xtable(FactorCor(tcq.fact.4), label="tab:tcq1fact4cor", caption="Four Factor TCQ correlations")
rownames(tcq4.cor.xtab) <- c("Acupuncture", "Cream", "Injections", "Pills")
colnames(tcq4.cor.xtab) <- c("Acupuncture", "Cream", "Injections", "Pills")
print(tcq4.cor.xtab)
@ 
...
tcq6.fact.xtab <- FactorXtab(tcq.fact.6,  names=c("Acu", "Cream", "Inj", "Pill", "Cred", "Exp"), label="tab:tcq1fact6", caption="Six Factor Solution, TCQ 1, Oblimin Rotation")
rownames(tcq6.fact.xtab) <- itemnames1
print(tcq6.fact.xtab, sanitize.text.function= function (x) x)
@ 


<<tcq6cor, echo=FALSE, results=tex>>=
#add names argument to FactorCor
print(xtable(FactorCor(tcq.fact.6,names=c("Acu", "Cream", "Inj", "Pill", "Cred", "Exp")), label="tab:tcq1fact6cor",caption="TCQ Six Factor Solution Correlations"))
@ 
...
print(xtable(mxCompare(base=tcq4fit, comp=c(tcq6fit)), label="tab:tcq1modelcomp", caption="CFA on TCQ Models, Sample One"), include.rownames=FALSE)
@ 
...
<<tcqconvgrmconstrained, echo=FALSE, results=hide>>=
tcqconv.grm.1pl <- grm(convtcq, constrained=TRUE, Hessian=TRUE)
...
print(xtable(coefirt(tcqconv.grm.1pl, se=TRUE), label="tab:tcq1convgrm1pl", caption="Coefficients for the TCQ 1 Conventional Scale, One Parameter Graded Response Model"), sanitize.colnames.function = function (x) x, scalebox=0.8)
@ 
Table \ref{tab:tcq1convgrm1pl}  shows the fit of the one parameter GRM. Note that the Cream credibility items are the most difficult, which fits with the lower mean credibility score on this variable observed in Table~\ref{tab:tcq1sumtotals}. The next step taken is to examine the fit of a more flexible model (a two parameter GRM, Appendix~\ref{cha:append-1:-supp}, Table~\ref{tab:tcq1convgrm2pl}, on Page~\pageref{tab:tcq1convgrm2pl}), and assess whether or not this represents a useful improvement over the model above.

<<tcqconvgrm2, echo=FALSE, results=tex, cache=TRUE>>=
tcqconv.grm.2pl <- grm(convtcq, constrained=FALSE, Hessian=TRUE)
tcqconv.grm2.coef <- coef(tcqconv.grm.2pl)

@ 
<<tcqconvgrm2plprint, echo=FALSE, results=hide>>=
print(xtable(coefirt(tcqconv.grm.2pl), label="tab:tcq1convgrm2pl", caption="Coefficients for TCQ 1 Conventional One Parameter Graded Response Model"))
@ 
<<tcqgrmcomp, echo=FALSE, results=hide, cache=TRUE>>=
grm.comp <- anova(tcqconv.grm.1pl, tcqconv.grm.2pl)
@ 
The result of this model comparison exercise showed that the 1 parameter model was a significantly better fit than the 2 parameter model ($p\le 0.001$).
The item information curves are perhaps more illuminating (Figure~\ref{fig:iictcqconv1grmpl}), as they show that the Injection items convey a large proportion of the information across the range when we allow separate discrimination parameters for individual items. 
<<iicggplotgen, echo=FALSE, results=hide>>=
tcqconv.grm.1pl.info <- extract_information(tcqconv.grm.1pl)
tcqconv.grm.2pl.info <- extract_information(tcqconv.grm.2pl)
@   


\begin{figure}
<<iccgrmtcqconv, echo=FALSE, fig=TRUE,pdf=TRUE, eps=TRUE, png=TRUE>>=
par(mfrow=c(1, 2))
plot(tcqconv.grm.1pl, type="IIC")
plot(tcqconv.grm.2pl, type="IIC")
## tcq1.1pl.pl <- ggplot_inf(tcqconv.grm.1pl.info)
## tcq1.2pl.pl <- ggplot_inf(tcqconv.grm.2pl.info)
## print(arrangeGrob(tcq1.1pl.pl+theme(legend.position="none"),tcq1.2pl.pl+theme(legend.position=c(1,1))))
@   
  \caption{Item Information Curves, TCQ Conventional Scale, One Parameter GRM (top) and Two Parameter GRM (bottom)}
  \label{fig:iictcqconv1grmpl}
\end{figure}

Next, the fit of the alternative TCQ (i.e. the acupuncture items) was examined using a series of graded response models.  
...
tcqalt.grm.constrained <- grm(na.omit(tcqalt), constrained=TRUE, Hessian=TRUE, start.val="random")
tcqalt.grm1.coef <- coef(tcqalt.grm.constrained)
@ 
<<tcaltgrm1plprint, echo=FALSE, results=tex>>=
print(xtable(coefirt(tcqalt.grm.constrained, se=FALSE), label="tab:tcq1altgrm1pl", caption="Coefficients for TCQ 1 Alternative Scale, One parameter Graded Response Model"), sanitize.colnames.function= function (x) x)
@ 
The model described in Table \ref{tab:tcq1altgrm1pl} has the following features. The discrimination parameter is reasonably high (reflecting that these items were endorsed less by many of the participants), and the parameter estimates are  closely distributed around zero.

<<tcqaltgrm2, echo=FALSE, results=tex, cache=TRUE>>=
tcqalt.grm2 <- grm(tcqalt, Hessian=TRUE, start.val="random")
...
print(xtable(coefirt(tcqalt.grm2, se=FALSE), label="tab:tcq1altgrm2pl", caption="Coefficient Estimates for TCQ 1 Alternative Scale, Two Parameter Graded Response Model"), sanitize.colnames.function= function (x) x, scalebox=0.7)
@ 
The model shown in Table~\ref{tab:tcq1altgrm2pl} has a much lower estimate of the discrimination of the individual items, counterbalanced by higher ability estimates for all of the item difficulty parameters. Note that the one parameter GRM did not converge properly, and so standard errors are not reported.  

<<tcqaltiicggplotgen, echo=FALSE, results=hide>>=
tcqalt.grm.1pl.info <- extract_information(tcqalt.grm.constrained)
tcqalt.grm.2pl.info <- extract_information(tcqalt.grm2)
@ 
\begin{figure}

<<iccgrmtcqalt, echo=FALSE, fig=TRUE,pdf=TRUE, eps=TRUE, png=TRUE>>=
par(mfrow=c(1, 2))
plot(tcqalt.grm.constrained, type="IIC")
plot(tcqalt.grm2, type="IIC")
@ 

@ 
  \caption{Item Information Curves, TCQ Alternative Scale, One Parameter GRM (top) and Two Parameter GRM (bottom)}
  \label{fig:iictcqalt1grmpl}
\end{figure}
...
print(xtable(tcq2.1.comp, caption="Comparison of Four and Six Factor Models for the TCQ1 on Sample Two", label="tab:tcq2testsemcomp"), include.rownames=FALSE)
@ 
...
grm.modcomp.xtab <- xtable(grm.modcomp, caption="Comparison of One and Two Parameter Graded Response Models, TCQ Conv, Split 2A", label="tab:grmmodcomp")
...
grm1plalt.done <- testIRTModels(tcqalt.grm.constrained, tcq2a.alt, grmconstraint=TRUE)r
m2plalt.done <- testIRTModels(tcqalt.grm.2pl, tcq2a.alt, grmconstraint=FALSE)
grm.modcomp <- rbind(grm1pl.done, grm2pl.done)
rownames(grm.modcomp) <- c("One Parameter GRM", "Two Parameter GRM")
print(xtable(grm.modcomp)) 
@ 
...
print(xtable(apademotables(tcq2.demo.full), caption="TCQ2 Summary Statistics", label="tab:tcq2sumstats"), include.rownames=FALSE)
@ 
...
The Health variable is quite skewed, as most respondents rated their health as very good as good, as can be seen from the mean and the median (Table~\ref{tab:tcq2sumstats}). 
...
The next step in the analysis was visualisation of data using scatter-plot matrices and conditioning plots. Figure \ref{fig:tcq2pairs}, the correlations between the scale totals can be seen. Notable are the high correlations between each of the conventional and alternative treatment methodologies, and their moderate correlations (in opposite directions) with the BMQ. Given the sample size, all correlations were significant at the $p \le 0.001$ level. 
@ 
\newpage
\begin{figure}
	\centering
...
sink("tmp.txt")
pairs.panels(tcqtotals, method="kendall", smooth=FALSE, ellipses=FALSE)
sink(NULL)
@ 
	\caption{Scatter-plot Matrix, TCQ 2. The upper triangle represents correlations between the variables, the lower triangle shows scatterplots between them, and the diagonal shows a histogram of the values. From top to bottom: Pilltot - Pill Credibility, Creamtot - Cream Credibility, Injtot - Injection Credibility, Acutot - Acupuncture Credibility, Homtot - Homeopathy credibility, Reitot - Reiki credibility, BMQ - BMQ Scores}
	\label{fig:tcq2pairs}
\end{figure}
\newpage
...
gendInj <- ggplot(na.omit(tcq2), aes(y=Injtot, x=Gender))+geom_boxplot()+xlab("")
...
college2 <- data.frame(name=c("Business and Law", "Arts", "Science", "Medicine and Health", "Central Admin"), shortname=c("Business", "Arts", "Science", "Medicine", "Admin"))
tcq.coll <- merge(tcq2, college2, by.x="College", by.y="name")
collpill <- ggplot(tcq.coll, aes(x=shortname, y=Pilltot))+geom_boxplot()+theme(axis.text.x=element_blank(),axis.title.x=element_blank())
collCream <- ggplot(tcq.coll, aes(x=shortname, y=Creamtot))+geom_boxplot()+theme(axis.text.x=element_blank(),axis.title.x=element_blank())
collInj <- ggplot(tcq.coll, aes(x=shortname, y=Injtot))+geom_boxplot()+theme(axis.text.x=element_blank(),axis.title.x=element_blank())
collAcu <- ggplot(tcq.coll, aes(x=shortname, y=Acutot))+geom_boxplot()+theme(axis.text.x=element_blank(),axis.title.x=element_blank())
collHom <- ggplot(tcq.coll, aes(x=shortname, y=Homtot))+geom_boxplot()+theme(axis.text.x=element_text(angle=90, hjust=1))+xlab("College")
collRei <- ggplot(tcq.coll, aes(x=shortname, y=Reitot))+geom_boxplot()+theme(axis.text.x=element_text(angle=90, hjust=1))+xlab("College")
...
print(xtable(expdf.gend, label="tab:experiencegend", caption="Experience with painkilling treatments by Gender"), include.rownames=FALSE)
@
...
print(xtable(inctab, label="tab:incomelevels", caption="Income Levels for Study two"), include.rownames=FALSE)
@ 
...
print(xtable(healthtab, label="tab:healthlevels", caption="Self Reported Health levels, Study Two"), include.rownames=FALSE)
@ 
...
Parallel analysis and the scree plot suggest that five factors seemed most optimal for this instrument. However, the MAP criterion disagrees, suggesting that seven factors might be more appropriate. To determine which of these criteria is correct, the five,  six and seven factor solutions were examined for interpretability and then subjected to CFA on the other splits of the data. This procedure was repeated for each of the other splits, and the coefficients of each of the models on each split were averaged and are reported below. 
...
print(FactorXtab(tcq2b.fact.5, names=c("Pill&Inj", "Cream", "Rei", "Hom", "Acu"),  label="tab:tcq2afact5",caption="Five factor Solution, TCQ 2 (Split B), Oblimin Rotation"), sanitize.text.function= function (x) x)
@ 


<<tcq2cfact5, echo=FALSE, results=hide>>=
tcq2c.fact.5 <- fa(na.omit(tcqall2c), nfactors=5, rotate="oblimin", fm="pa")
print(FactorXtab(tcq2c.fact.5, names=c("PillInj", "Hom", "Rei", "Cream", "Acu"), label="tab:tcq2cfact5",caption="Five factor Solution, TCQ 2 (Split C), Oblimin Rotation"), sanitize.text.function= function (x) x)
@ 
...
tcq2c5.av.xtab <- xtable(tcq2c5.av, label="tab:tcq2fact5av", caption="Average of Five Factor Solutions over Splits B and C")
align(tcq2c5.av.xtab) <- c("l", rep("r", ncol(tcq2c5.av.xtab)))
rownames(tcq2c5.av.xtab) <- itemnames
print(tcq2c5.av.xtab)
@ 



The average of the two five factor solutions (Appendix~\ref{cha:append-1:-supp}, Table~\ref{tab:tcq2fact5av}, Page~\pageref{tab:tcq2fact5av}) merged the Pill credibility and Injection credibility items together, and this pattern was repeated across both splits. Note that both the Cream and Homeopathy factors have slightly lower loadings than do the others, and the Pill \& Injections factor is weighted more towards Injections.  
...
print(FactorXtab(tcq2b.fact.6, names=c("Rei", "Hom", "Acu", "Cream", "Inj", "Pill"),label="tab:tcq2bfact6",caption="Six factor Solution, TCQ 2, Oblimin Rotation"), sanitize.text.function= function (x) x)
@ 


<<tcq2cfact6, echo=FALSE, results=hide>>=
tcq2c.fact.6 <- fa(na.omit(tcqall2c), nfactors=6, rotate="oblimin", fm="pa")
print(FactorXtab(tcq2c.fact.6,names=c("Hom", "Rei", "Acu", "Cream", "Inj", "Pill"), label="tab:tcq2cfact6",caption="Six factor Solution, TCQ 2, Split C,  Oblimin Rotation"), sanitize.text.function= function (x) x)
@ 

<<tcq2dfact6, echo=FALSE, results=hide>>=
tcq2d.fact.6 <- fa(na.omit(tcqall2d), nfactors=6, rotate="oblimin", fm="pa")
print(FactorXtab(tcq2d.fact.6, names=c("Reiki", "Hom", "Acu", "Cream", "Inj", "Pill"), label="tab:tcq2dfact6",caption="Six factor Solution, TCQ 2(Split D), Oblimin Rotation"), sanitize.text.function= function (x) x)
@ 
...
tcq2.fact6.av <- FactorAverage(sols=list(tcq2b.fact6.n, tcq2c.fact6.n, tcq2d.fact6.n), mynames=c("Hom", "Rei", "Acu", "Cream", "Inj", "Pill", "Communalities" ), FUN=mean)
rownames(tcq2.fact6.av) <- itemnames
print(xtable(tcq2.fact6.av, label="tab:tcq2fact6average",caption= "Average of Six Factor Solutions over Splits B, C and D"), sanitize.text.function = function (x) x) ##this is wrong, something weird happened here
@ 
...
<<tcq2factorcoraverage, echo=FALSE, results=tex>>=
tcq2.6.average.cor <- FactorAverage(sols=list(tcq2b.fact6.n, tcq2c.fact6.n, tcq2d.fact6.n), mynames=c("Pill", "Cream", "Inj", "Acu", "Hom", "Rei"), correlations=TRUE)
print(xtable(tcq2.6.average.cor, caption="Average of Factor Correlations, Six Factor Solution, TCQ2 (B,C,D)", label="tab:tcq26factorcoraverage"), sanitize.text.function = function (x) x)
@ 

Table~\ref{tab:tcq26factorcoraverage} shows that the three conventional treatment factors and the three alternative factors correlated with each other but tended not to correlate highly with the factors of the opposite modality. 
...
print(FactorXtab(tcq2b.fact.7, names=c("Hom", "Rei", "Acu", "Cream","Inj", "Pill", "Cred"), label="tab:tcq2bfact7",caption="Seven Factor Solution, TCQ 2B, Oblimin Rotation"), sanitize.text.function= function (x) x)
@ 


<<tcq2cfact7, echo=FALSE, results=hide>>=
tcq2c.fact.7 <- fa(na.omit(tcqall2c), nfactors=7, rotate="oblimin", fm="pa")
print(FactorXtab(tcq2c.fact.7, names=c("Hom", "Rei", "Acu", "Cream", "Inj", "Pill", "Cred"), label="tab:tcq2cfact7",caption="Seven Factor Solution, TCQ 2, Split C,  Oblimin Rotation"), sanitize.text.function= function (x) x)
@

<<tcq2dfact7, echo=FALSE, results=hide>>=
tcq2d.fact.7 <- fa(na.omit(tcqall2d), nfactors=7, rotate="oblimin", fm="pa")
print(FactorXtab(tcq2d.fact.7, names=c("Reiki", "Hom", "Acu", "Cream", "Inj", "Pill", "Cred"), label="tab:tcq2dfact7",caption="Seven Factor Solution, TCQ 2, Oblimin Rotation (Split D)"), sanitize.text.function= function (x) x)
@ 

<<tcqomegas, echo=FALSE, results=hide>>=
tcq2b6.omega <- omega(tcq2b.fact.6$r.scores, nfactors=2)
tcq2b7.omega <- omega(tcq2b.fact.7$r.scores, nfactors=2)
tcq2c6.omega <- omega(tcq2c.fact.6$r.scores, nfactors=2)
tcq2c7.omega <- omega(tcq2c.fact.7$r.scores, nfactors=2)
tcq2d6.omega <- omega(tcq2d.fact.6$r.scores, nfactors=2)
tcq2d7.omega <- omega(tcq2d.fact.7$r.scores, nfactors=2)
@ 
...
print(xtable(tcq2.fact7.av, label="tab:tcq2fact7average", caption="Average of Seven Factor Solutions over Splits B, C and D"), sanitize.text.function = function (x) x, table.placement="ht")
tcq2.fact7.average.cor <- FactorAverage(sols=list(tcq2b.fact7.n, tcq2c.fact7.n, tcq2d.fact7.n), mynames=c("Pill", "Cream", "Inj", "Acu", "Hom", "Rei", "Cred"), correlations=TRUE)
@ 
...
<<tcq27factorcor, echo=FALSE, results=tex>>=
print(xtable(tcq2.fact7.average.cor, label="tab:tcq27factorcoraverage", caption="Averaged Factor Correlations for the Seven Factor Solution, Sample Two"), sanitize.text.function = function (x) x)
@ 

The factor correlations are shown in Table~\ref{tab:tcq27factorcoraverage}, and follow a similar pattern to the six factor correlations. Note that the Pill and Injection factors correlate much more highly with the Credibility factor, which is a point that will be revisited below. 
...
\begin{figure}
<<omegagraph, echo=FALSE, fig=TRUE, pdf=TRUE>>=
print(omega.graph(tcq2b6.omega, labels=c("Rei", "Hom", "Acu", "Cream", "Inj", "Pill")))
@   
  \caption{Hierarchical Factor Structure (Omega) for TCQ Data}
  \label{fig:tcqomega}
\end{figure}

Figure~\ref{fig:tcqomega} shows that the hierarchical solution breaks down into a conventional and alternative structure (both factors constrained to be equal). This suggests that modelling the data with two higher-order structures might be useful. However, this failed to provide a parameter-adjusted decrease in log-likelihood, and so was not pursued further. 
...
print(xtable(tcq2.notC.semcompare, label="tab:tcq2notCsemcompare", caption="TCQ Models from Split C tested against Splits A, B and D"), scalebox=0.9, include.rownames=FALSE)
@ 
...
print(xtable(tcq2.notD.semcompare, label="tab:tcq2notDsemcompare", caption="TCQ 2 Models built on Split D, tested against Splits A, B and C"), scalebox=0.9, include.rownames=FALSE)
@ 


<<tcq2notBsemcompare, echo=FALSE, results=tex>>=
tcq2.notB.semcompare <- mxCompare(tcq5fit.notB,c(tcq6fit.notB, tcq7fit.notB))
print(xtable(tcq2.notB.semcompare, label="tab:tcq2notBsemcompare", caption="TCQ 2 Factor Solutions From Split B, Tested Against Splits A, C and D"), scalebox=0.9, include.rownames=FALSE)
@ 
...
Next, the assumptions were checked for the conventional items, and there were no violations for either the IIO or monotonicity assumptions. 
...
tcq2b.conv.grm.1pl <- grm(tcq2b.conv, constrained=TRUE, Hessian=TRUE)
tcq2b.conv.grm.2pl <- grm(tcq2b.conv, constrained=FALSE, Hessian=TRUE)
@ 

<<tcq2cconvgrm, echo=FALSE, results=hide, cache=TRUE>>=
tcq2c.conv.grm.1pl <- grm(tcq2c.conv, constrained=TRUE, Hessian=TRUE)
tcq2c.conv.grm.2pl <- grm(tcq2c.conv, constrained=FALSE, Hessian=TRUE)
@ 

<<tcq2dconvgrm, echo=FALSE, results=hide>>=
tcq2d.conv.grm.1pl <- grm(tcq2d.conv, constrained=TRUE, Hessian=TRUE)
tcq2d.conv.grm.2pl <- grm(tcq2d.conv, constrained=FALSE, Hessian=TRUE)
@ 

<<tcqgrm1plaverage, echo=FALSE, results=hide>>=
tcq2.conv.grm.1pl.av <- irtAverage(sols=list(tcq2b.conv.grm.1pl, tcq2c.conv.grm.1pl, tcq2d.conv.grm.1pl), se=TRUE)
tcq2.conv.grm.2pl.av <- irtAverage(sols=list(tcq2b.conv.grm.2pl, tcq2c.conv.grm.2pl, tcq2d.conv.grm.2pl), se=FALSE) #hessian matrix not positive definite
@ 

<<tcqconvgrm1plaverage, echo=FALSE, results=tex>>=
print(xtable(coefirt(tcq2.conv.grm.1pl.av, se=TRUE, averaged=TRUE), label="tab:tcq2grm1plaverage", caption="Item Difficulty Thresholds for TCQ Conventional items averaged over Splits B, C and D"), sanitize.colnames.function= function (x) x, scalebox=0.9)
@ 
...
print(xtable(coefirt(tcq2.conv.grm.2pl.av, averaged=TRUE), label="tab:tcq2convgrm2plaverage", caption="Item Difficulty Thresholds for Two Parameter GRM, TCQ Conv, Averaged over Splits B, C and D"), sanitize.colnames.function = function (x) x)
@ 

Table~\ref{tab:tcq2convgrm2plaverage} shows the estimated difficulty parameters for the two parameter GRM averaged across Splits B, C and D. Note that with this model, the cream credibility items have increased in difficulty, but the injection items have the highest discrimination, suggesting that these items are the best for distinguishing between high and low ``ability'' respondents. Standard errors are not shown as the models did not converge for Splits B and D. 
<<tcqaltiicggplotgen2, echo=FALSE, results=hide>>=
tcqconv2d.grm.1pl.info <- extract_information(tcq2d.conv.grm.1pl)
tcqconv2d.grm.2pl.info <- extract_information(tcq2d.conv.grm.2pl)
@ 
\begin{figure}

<<iccgrmtcqconv2d, echo=FALSE, fig=TRUE,pdf=TRUE, eps=TRUE, png=TRUE>>=
par(mfrow=c(1, 2))
plot(tcq2d.conv.grm.1pl, type="IIC")
plot(tcq2d.conv.grm.2pl, type="IIC")
@

  \caption{Item Information Curves, TCQ Conventional, Split 2, One Parameter GRM (top) and Two Parameter GRM (bottom)}
  \label{fig:iicgrmtcqconv2pl}
\end{figure}

Figure~\ref{fig:iicgrmtcqconv2pl} shows that for the second sample, both the Cream and Injection items contributed a far greater proportion of the information when their slopes were allowed to vary independently. Interestingly enough, this pattern did not appear in Split B, suggesting that sampling variance was at play here. 
...
tcq2b.alt.grm.1pl <- grm(tcq2b.alt, constrained=TRUE, Hessian=TRUE)
tcq2b.alt.grm.2pl <- grm(tcq2b.alt, constrained=FALSE, Hessian=TRUE)
@ 

<<tcq2caltgrm, echo=FALSE, results=hide, cache=TRUE>>=
tcq2c.alt.grm.1pl <- grm(tcq2c.alt, constrained=TRUE, Hessian=TRUE)
tcq2c.alt.grm.2pl <- grm(tcq2c.alt, constrained=FALSE, Hessian=TRUE)
@ 

<<tcq2daltgrm, echo=FALSE, results=hide, cache=TRUE>>=
tcq2d.alt.grm.1pl <- grm(tcq2d.alt, constrained=TRUE, Hessian=TRUE)
tcq2d.alt.grm.2pl <- grm(tcq2d.alt, constrained=FALSE, Hessian=TRUE)
@ 
...
print(xtable(coefirt(tcq2.alt.grm.1pl.av), label="tab:tcq2altgrm1plaverage", caption="Item Difficulty Thresholds for One Parameter GRM on the Alternative Items, averaged over Splits B, C and D"), sanitize.colnames.function= function (x) x)
@ 

Table~\ref{tab:tcq2altgrm1plaverage} shows the averaged coefficients for the one parameter alternative item GRM. It can be seen that the Acupuncture items had the lowest difficulty thresholds, but all items had extremely high discrimination parameters. 
...
print(xtable(coefirt(tcq2.alt.grm.2pl.av), label="tab:tcq2altgrm2plaverage", caption="Item Difficulty Thresholds for Two Parameter GRM on the Alternative Items, averaged over Splits B, C and D"))
@ 
...
The next stage in the analysis is to take the best of these models from each split and test them on the entire dataset. While this is, in a sense, analysing the data twice, it is the most practicable way in which to determine the best model for the entire sample\footnote{due to the changes in the scale in Sample Two, a back testing strategy was not possible in this case}. 
...
print(xtable(tcqall2.semcompare, label="tab:tcqall2semcompare", caption="TCQ2 Models from each split tested against the full dataset"), scalebox=0.8, include.rownames=FALSE)
@ 
...
tcq2b.conv.grm.1pl.s <- grm(tcq2b.conv.s, constrained=TRUE, Hessian=TRUE)
tcq2c.conv.grm.1pl.s <- grm(tcq2c.conv.s, constrained=TRUE, Hessian=TRUE)
tcq2d.conv.grm.1pl.s <- grm(tcq2d.conv.s, constrained=TRUE, Hessian=TRUE)
tcq2b.alt.grm.1pl.s <- grm(tcq2b.alt.s, constrained=TRUE, Hessian=TRUE)
tcq2c.alt.grm.1pl.s <- grm(tcq2c.alt.s, constrained=TRUE, Hessian=TRUE)
tcq2d.alt.grm.1pl.s <- grm(tcq2d.alt.s, constrained=TRUE, Hessian=TRUE)
...
\section{Relationships betwen Credibility Totals and Other Variables}
\label{sec:regression-analyses}
...
print(xtable(apareg(pilltotlm.holdout), label="tab:tcq2pillholdout", caption="Coefficients for Pill credibility Regression on Held Out Data"), sanitize.colnames.function= function (x) x)
@ 


From  the model on the held out data, it can be seen that the results (Table~\ref{tab:tcq2pillholdout}) were highly significant F(10,129)=54.76, $p\le 0.001$. The model had an adjusted $R^2$ of 0.51, which equates to approximately 25\% of the variance in Pill Credibility explained. The Cream and Injection variables were highly significant, while the Acupuncture and Experience with Pills variables did not pass the traditional significance filter. Experience with creams did pass this filter however, which may be a function of increasing experience with multiple painkilling problems increasing their credibility.  
...
print(xtable(apareg(creamtotlm.holdout), label="tab:tcq2creamholdout", caption="Coefficients for Cream credibility Regression on Held Out Data"), sanitize.colnames.function = function (x) x)
@ 
...
The results for the held out data are shown in Table~\ref{tab:tcq2creamholdout}. The model was again significant, with an $F(6,144)=20.61$, and a p-value of $p\le 0.001$. The adjusted $R^2$ for the model was equal to 0.4497. Pill credibility, experience with  cream treatments, Injection and Acupuncture credibility remained significant,
...
print(xtable(apareg(injtotlm.holdout), label="tab:tcq2injholdout", caption="Coefficients for Injection credibility Regression on Held Out Data"), sanitize.colnames.function = function (x) x)
@ 
...
print(xtable(apareg(acutotlm.holdout), label="tab:tcq2acuholdout", caption="Coefficients for Acupuncture credibility Regression on Held Out Data"), sanitize.colnames.function = function (x) x)
@ 

As shown in Table \ref{tab:tcq2acuholdout}, only Homeopathy and Reiki, remained significant on the unseen data. 
...
Following this, the same procedure was applied to the Homeopathy credibility totals. 
...
print(xtable(apareg(homtotlm.holdout), label="tab:tcq2homholdout", caption="Coefficients for Homeopathy credibility Regression on Held Out Data"), sanitize.colnames.function = function (x) x)
@ 

Table~\ref{tab:tcq2homholdout} shows the estimated coefficients from the stepwise selected model on unseen data. It can be seen that Acupuncture and Reiki credibility remain significant, along with the relevant experience variables for each form of alternative credibility. The model has an $R^2$ of 0.62, which is extremely high. 
...
%% Table \ref{tab:tcq2ahomridge} shows a similar pattern to the lasso fit described earlier. 
 ...
 The selection and testing procedure was then applied to the Reiki totals. 
...
print(xtable(apareg(reitotlm.holdout), label="tab:tcq2reiholdout", caption="Coefficients for Reiki credibility Regression on Held Out Data"), sanitize.colnames.function = function (x) x)
@ 

Table \ref{tab:tcq2reiholdout} shows the performance of the stepwise selected coefficients on unseen data. It can be seen that the only coefficients which maintained significance were those for Homeopathy and Acupuncture credibility, while the $R^2$ was quite low, at approximately 0.4. 
...
Additionally, this factor structure has been validated by the use of unseen data to test and compare all models. This allows for maximum use to be made of the limited data collected in the experimental research, and is an approach which is not often employed within the field. 
...
That being said, there are some important limitations to the study. Firstly, there was no behavioural or observational outcome to benchmark the test against. Secondly, although this study sampled twice from the student population and once from the staff population, this test still relies entirely on the responses of staff and students at one particular university. Further replication of the factor structure in heterogeneous groups is warranted before extensive use of the instrument takes place. 
