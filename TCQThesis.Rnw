...
pilltotlm1 <- lm(Pilltot~Creamtot+Injtot+Acutot+Homtot+Reitot+Expalt+Expconv+Gender+Income+Health, data=tcq2nota.complete)
pilltot.step <- stepAIC(object=pilltotlm1, upper=pilltotlm1,lower=~1, direction="both")
pilltot.pred <- predict(pilltotlm1, newdata=tcq2a)
@ 

When tested against the out of sample data, the predicted data had a correlation of 0.64 with the actual data, which is an acceptable level of prediction (especially with a linear model). 
