...
rownames(tcq4.cor.xtab) <- c("Acupuncture", "Cream", "Inj", "Pills")
colnames(tcq4.cor.xtab) <- c("Acupuncture", "Cream", "Inj", "Pills")
print(tcq4.cor.xtab)
@ 
...
The factor correlations are shown in Table~\ref{tab:tcq27factorcoraverage}, and follow a similar pattern to the six factor correlations. Note that the Pill and Inj factors correlate much more highly with the Credibility factor, which is a point that will be revisited below. 
...
print(xtable(coefirt(tcq2.conv.grm.2pl.av, averaged=TRUE), label="tab:tcq2convgrm2plaverage", caption="Item Difficulty Thresholds for Two Parameter GRM, Averaged over Splits B, C and D"), sanitize.colnames.function = function (x) x)
@ 
